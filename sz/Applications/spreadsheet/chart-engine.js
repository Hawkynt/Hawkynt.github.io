;(function() {
  ...
  function _showChartOptionsDialog(chart) {
    const title = prompt('Chart title:', chart.options.title || '');
    if (title !== null) {
      chart.options.title = title;
      const addTrend = confirm('Add trendline? (OK = yes)');
      if (addTrend) {
        const tType = prompt('Trendline type (linear/exponential/polynomial):', 'linear') || 'linear';
        chart.options.trendline = { type: tType };
        if (tType === 'polynomial') {
          const deg = prompt('Polynomial degree (2 or 3):', '2');
          chart.options.trendline.degree = parseInt(deg, 10) || 2;
        }
      } else
        delete chart.options.trendline;

      const addLabels = confirm('Show data labels? (OK = yes)');
      if (addLabels)
