;(function() {
      ...
      domEl.addEventListener('contextmenu', (e) => {
        ...
        if (element.type === 'textbox' && PresentationsApp.SpellCheck) {
          const handled = PresentationsApp.SpellCheck.showContextMenu(e, domEl, element, () => {
            // On replace: sync content back
            const inner = domEl.querySelector('[contenteditable]');
            if (inner && element)
              element.content = inner.innerHTML;
            markDirty();
          });
          // Do not prevent default if not handled -- allow normal context menu
        }

        // Image context menu with Edit Alt Text
        if (element.type === 'image') {
          ...
          const items = [
            { label: 'Edit Alt Text...', action: () => showAltTextDialog() },
            { sep: true },
  ...
  function handleElementDblClick(e, domEl, elId) {
    ...
    if (element.type === 'textbox') {
      // Focus the contenteditable inner
      const inner = domEl.querySelector('[contenteditable]');
      if (inner)
        inner.focus();
  ...
  function previewTransition() {
    ...
    if (!engine) {
      // Fallback: simple opacity flash
      slideCanvas.style.opacity = '0';
      slideCanvas.style.transition = 'opacity 0.3s ease';
      setTimeout(() => {
        slideCanvas.style.opacity = '1';
        setTimeout(() => { slideCanvas.style.transition = ''; }, 300);
      }, 300);
      return;
    }

    // Create temporary front/back layers inside the viewport for real preview
    const viewport = canvasViewport;
    if (!viewport)
      return;

    const frontLayer = document.createElement('div');
    frontLayer.style.cssText = 'position:absolute;top:0;left:0;width:100%;height:100%;z-index:10;pointer-events:none;';
    const backLayer = document.createElement('div');
    backLayer.style.cssText = 'position:absolute;top:0;left:0;width:100%;height:100%;z-index:9;pointer-events:none;';

    // Clone canvas content into front layer
    ...
    // Back layer shows original canvas (which stays in place)
    viewport.style.position = 'relative';
    viewport.appendChild(frontLayer);
    viewport.appendChild(backLayer);

    // Hide original canvas during preview
    ...
    const finishDelay = duration * 1000 + 100;
    setTimeout(() => {
      // Cleanup
      engine.cancelPending(subTimers);
      for (const id of subTimers)
        clearInterval(id);
      if (frontLayer.parentNode)
        frontLayer.parentNode.removeChild(frontLayer);
      if (backLayer.parentNode)
        backLayer.parentNode.removeChild(backLayer);
  ...
  function renderSorterView() {
    ...
    for (let i = 0; i < presentation.slides.length; ++i) {
      ...
      slideSorterEl.appendChild(thumb);
    }
  }
  ...
  function populateTransitionGallery() {
    ...
    // Try to get transitions from parent TransitionEngine, fallback to hardcoded list
    const engineTransitions = window.parent?.SZ?.TransitionEngine?.ALL_TRANSITIONS;
    const transitions = engineTransitions || ALL_TRANSITIONS;
