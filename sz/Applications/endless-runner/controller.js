;(function() {
  ...
  const OBSTACLE_SPAWN_RANGE = 120;
  ...
  function update(dt) {
    ...
    skylineOffset = (skylineOffset + currentSpeed * dt * 2) % CANVAS_W;
  ...
  function draw() {
    ...
    // City skyline silhouette (parallax layer behind everything)
    {
      ...
      for (let i = 0; bx < CANVAS_W + 50; ++i) {
        ...
        if (b.h > 50 && (tier.name === 'night' || tier.name === 'neon')) {
          ...
          ctx.fillStyle = tier.name === 'neon' ? 'rgba(30,10,60,0.7)' : 'rgba(10,10,30,0.6)';
        }
        bx += b.w + 6;
      }
      ctx.restore();
    ...
    // Guardrail posts along road edges
    {
      ...
      for (let py = -postOffset; py < CANVAS_H; py += postSpacing) {
        // Left guardrail post
  ...
  canvas.addEventListener('pointerdown', function(e) {
    ...
    // Touch controls: left third = left, right third = right, middle = jump
