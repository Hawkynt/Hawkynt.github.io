...
- [x] As a player, I want to maximize the game window to fill the desktop so I can see a larger farm view and more detail
- [x] As a player, I want a tooltip system that shows contextual information when hovering over crops, livestock, shop items, and UI elements so I understand costs, growth times, and statuses at a glance
...
- **Drag selection** — click and drag to select multiple tiles for batch planting or harvesting; per-tile highlights indicate action (green=plant, gold=harvest, red=no action); dashed selection border
- **Tutorial overlay** — 2-page guide shown on first play, toggled with H key, persisted via localStorage
- **Window maximize support** for a larger farm view with responsive canvas resizing
- **Tooltip system** showing contextual information on hover for crops, livestock, shop items, and UI elements
