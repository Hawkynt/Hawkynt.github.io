;(function() {
  ...
  function update(dt) {
    ...
    // Parallax scrolling — modulo must match tile spacing to avoid seam jumps
    layerOffsets[0] = (layerOffsets[0] + speed * PARALLAX_LAYERS[0].speed * step) % 60;
    layerOffsets[1] = (layerOffsets[1] + speed * PARALLAX_LAYERS[1].speed * step) % 60;

    cloudOffset = (cloudOffset + speed * 0.15 * step) % (CANVAS_W + 200);
    hillsOffset = (hillsOffset + speed * 0.45 * step) % 80;
  ...
  function draw() {
    ...
    // Parallax background layers — first layer (distant mountains)
    {
      ...
      for (let x = -off - 60; x < CANVAS_W + 60; x += 60) {
        ctx.beginPath();
    ...
    for (let x = -hillsOffset - 80; x < CANVAS_W + 80; x += 80) {
      ctx.beginPath();
