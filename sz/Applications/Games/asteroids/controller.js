;(function() {
  ...
  function update(dt) {
    ...
    if (ship) {
      ...
      /* Mouse: rotate ship toward cursor */
      if (mouseActive) {
        const target = Math.atan2(mouseY - ship.y, mouseX - ship.x);
        let diff = target - ship.angle;
        while (diff > Math.PI) diff -= Math.PI * 2;
        while (diff < -Math.PI) diff += Math.PI * 2;
        const turn = Math.min(Math.abs(diff), SHIP_ROTATION_SPEED) * Math.sign(diff);
        ship.angle += turn;
      }
      ...
      /* Auto-fire on held space or held mouse button (not during warp) */
      if ((keys[' '] || mouseDown) && !warpState) {
  ...
  function drawModeSelect() {
    ...
    ctx.font = scaledFont(11, 'normal');
    ctx.textAlign = 'center';
    ctx.fillText('Click a mode or press 1\u20134 to start  \u00b7  \u25C0 \u25B6 field size', W / 2, footerY);
    ctx.fillText('Mouse aims + clicks to shoot \u00b7 Arrow keys/WASD + Space also work', W / 2, footerY + 16 * uiS);
  }
  ...
  canvas.addEventListener('pointerdown', (e) => {
    ...
    if (modeSelectActive) {
      ...
      for (let i = 0; i < modes.length; ++i) {
        const y = startY + i * (cardH + gap);
        if (pos.x >= cardX && pos.x <= cardX + cardW && pos.y >= y && pos.y <= y + cardH) {
          modeSelectActive = false;
          newGame(modes[i]);
          return;
        }
      ...
      /* Click field size arrows */
      const fieldY = 125 * uiS;
      if (pos.y >= fieldY - 15 * uiS && pos.y <= fieldY + 25 * uiS) {
        if (pos.x < W / 2)
          cycleFieldSize(-1);
        else
          cycleFieldSize(1);
