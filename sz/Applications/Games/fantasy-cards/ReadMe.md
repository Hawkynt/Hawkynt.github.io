...
- **55 unique cards** across 5 types: Attack, Defend, Heal, Buff, Debuff — each with detailed descriptions and exact numeric effects
- **Card type color coding** — Attack (red), Defend (blue), Heal (green), Buff (gold), Debuff (purple) with gradient card backgrounds
- **Card hover tooltip** — hover any card to see full details including mana cost, power, and flavor text
- **How to Play tutorial** — multi-page overlay explaining game rules, card types, and strategy tips
- **Enhanced card rendering** — gradient backgrounds, art areas, mana gem icons, power badges, type indicators
- **Improved deck builder** — shows card details, visual distinction by type, card counts
- **Improved menu screen** — card type legend, game info, visual polish
...
- **Mouse click to start** — click anywhere on the menu screen to begin the game
