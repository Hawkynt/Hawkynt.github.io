;(function(root, factory) {
  ...
  SZ.System.Drawing.IconCodec = api;
  ...
  function decodeBmpEntry(data, wHint, hHint, bppHint, options = {}) {
    ...
    const outPalette = palette.length > 0
      ? palette.map(p => [p[0], p[1], p[2], 255])
      : null;
    return { imageData, bpp: actualBpp, palette: outPalette };
  }
  ...
  function encodeBmpEntry(img, options = {}) {
    ...
    const headerSize = 40;
    const paletteSize = bpp <= 8 ? (1 << bpp) : 0;
    ...
    view.setUint32(32, paletteSize, true);

    const ensurePalette = options.ensureImagePalette || function(image) {
      if (!image.palette || image.palette.length < paletteSize) {
        image.palette = defaultPaletteForBpp(bpp) || [];
      }
    };
    const defaultPalette = options.createDefaultPaletteForBpp || defaultPaletteForBpp;
    const nearestIndex = options.nearestPaletteIndex || nearestPaletteIndex;

    let palette = null;
    if (paletteSize > 0) {
      ensurePalette(img);
      palette = (img.palette || defaultPalette(bpp)).slice(0, paletteSize);
    ...
    for (let y = 0; y < h; ++y) {
      ...
      for (let x = 0; x < w; ++x) {
        ...
        if (bpp === 32) {
          ...
          if (idx == null) {
            idx = nearestIndex(palette, r, g, b, 255);
          ...
          if ((x & 1) === 0)
            buf[byteOff] = (buf[byteOff] & 0x0f) | ((idx & 0x0f) << 4);
          else
            buf[byteOff] = (buf[byteOff] & 0xf0) | (idx & 0x0f);
        } else if (bpp === 1) {
          const key = (r << 16) | (g << 8) | b;
          let idx = keyToIndex.get(key);
          if (idx == null) {
