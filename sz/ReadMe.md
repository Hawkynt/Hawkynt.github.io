...
- [x] Space Combat: Top-down space shooter on canvas with 5 weapon types (spread shot, laser, homing missile, plasma AOE, piercing beam), 5 persistent upgrade categories (extra projectiles, damage boost, speed boost, fire rate, shield capacity) that accumulate across waves up to level 3 each, 7 enemy types with wave-gated introduction and distinct AI (scouts wander, fighters shoot, darts dive-bomb the player, bombers are slow and tough, snipers stay high with accurate long-range shots and aim lines, carriers spawn mini scouts, juggernauts fire spreads with scaling HP), HP bars on multi-HP enemies, hit flash feedback, type-specific ship shapes (triangles, diamonds, hexagons), star-shaped persistent upgrade pickups vs circular weapon pickups, kill streak scoring (x2-x8 multiplier), shield system with wave recharge, wave-based progression with increasing enemy variety and speed scaling, visual effects (plasma explosions, beam trails, sniper aim lines, carrier bay indicators, juggernaut cross marks), persistent high scores
- [x] Card Games Suite: 25-game card collection in a single app — Solitaire, FreeCell, Spider, Pyramid, Golf, TriPeaks (solitaire), Poker, Blackjack (casino), Uno, Skip-Bo, Crazy Eights, Mau-Mau (shedding), Skat, Doppelkopf, Hearts, Spades (trick-taking), Canasta, Gin Rummy (rummy), War, Memory, Go Fish, Old Maid, Snap (simple), Schwimmen, Durak (European). 3-column game selection menu, mouse-only clickable action buttons for all games, canvas "Menu" button for returning to game selection, "Back to Menu" in menu bar, card-deal animations, card-flip transitions, winning-hand glow effects, chip sparkle particles, floating text, screen shake, persistent high scores per variant
...
51 applications are currently implemented (5 system + 46 user apps). See [docs/appideas.md](docs/appideas.md) for the full application idea list with hosting format (iframe vs hosted) and status tracking.
...
- Tower Defense expansion: Added 4 new tower types (Poison/Tesla/Mortar/Flame = 9 total), 3 new enemy types (Healer/Swarm/Shield = 8 total), auto-wave mode, mouse-only controls, pre-wave warnings with path visualization, improved tower/enemy rendering
...
- Touch/mouse input for all games: click/tap-to-start added to 12 keyboard-only games (tetris, snake, pacman, bomberman, space-mining, space-horror, space-racing, platformer, boulder-dash, bubble-bobble, asteroids, space-invaders); swipe controls for Snake, Pac-Man (swipe to change direction), and Tetris (swipe to move, tap to rotate, swipe up for hard drop); all overlay texts updated to mention tap/click
- Space Combat content expansion: 4 new weapons (shotgun, lightning, flak, railgun = 9 total), 4 new enemy types (cloaker, minelayer, swarm, shieldbearer = 11 total), boss system every 5 waves (Commander/Destroyer/Mothership/Dreadnought with enrage), 2 new persistent upgrades (bullet size, magnet range), max upgrade level 5, max multiplier 12
