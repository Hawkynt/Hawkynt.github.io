...
- Spreadsheet: Office parity upgrade — XLSX as native default save format with enhanced style/merge/freeze round-trip, auto-fill drag with pattern detection (numbers, days, months, text+number, formulas), data validation (list/number/text-length with dropdown UI), pivot tables with aggregation engine and config dialog, 11 chart types as inline draggable/resizable charts, sparklines (line/column/win-loss) rendered in cells
