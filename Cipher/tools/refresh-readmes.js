...
const TABLE_BEGIN = '<!-- BEGIN GENERATED CATEGORY TABLE -->';
const TABLE_END = '<!-- END GENERATED CATEGORY TABLE -->';
...
function renderList(entries) {
  ...
  const lines = [BEGIN, '', 'Generated from the registry by `tools/refresh-readmes.js`.', ''];
...
// The top-level README used to open with a headline count, which reads as a
// boast, dates the moment anything is added, and tells a reader nothing about
// what is actually in here. A plain inventory is more use and cannot go stale.
function renderCategoryTable(byDirectory) {
  const rows = [...byDirectory]
    .map(([directory, entries]) => ({ directory, count: entries.length }))
    .sort((a, b) => a.directory.localeCompare(b.directory));
  const lines = [TABLE_BEGIN, '', 'Generated by `tools/refresh-readmes.js`.', '',
    '| Directory | Algorithms |', '| --- | ---: |'];
  for (const row of rows) lines.push(`| [\`algorithms/${row.directory}/\`](algorithms/${row.directory}/) | ${row.count} |`);
...
function main() {
  ...
  // The top-level README carries the per-directory inventory.
  const topLevel = path.join(CIPHER_ROOT, 'README.md');
  const topBefore = fs.readFileSync(topLevel, 'utf8');
  if (topBefore.includes(TABLE_BEGIN)) {
