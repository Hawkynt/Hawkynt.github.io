...
// The exemption list is shared with TestEngine, which gates on it too.
const { ROUND_TRIP_EXEMPT } = require('./round-trip-exemptions.js');
...
function checkCipher(algorithm, corpus, vector) {
  ...
  for (const testCase of corpus) {
    ...
    if (sameBytes(restored, testCase.data)) {
      accepted++;
      if (testCase.data.length > 0) acceptedNonEmpty++;
  ...
  // Counting the empty case as evidence lets an algorithm that throws on every
  // real input still pass: no case disagrees, and the empty one round-trips
  // through any implementation that returns [] for []. That is not a property of
  // the algorithm, so at least one case carrying actual bytes has to survive.
  if (!problems.length && acceptedNonEmpty === 0)
    problems.push(accepted === 0
      ? `refused all ${refused} case(s) built from its own vector; it accepts nothing`
      : `refused all ${refused} case(s) carrying data; only the empty input round-tripped, `
        + `which every implementation that returns [] for [] does regardless of what it computes`);
