/*
 * Bcrypt-PBKDF (OpenBSD PBKDF) Implementation
 * Production-ready key derivation function using eksblowfish (expensive key schedule Blowfish)
 * (c)2006-2025 Hawkynt
 *
 * Based on OpenBSD's bcrypt_pbkdf and Botan library implementation.
 * This is NOT standard Bcrypt password hashing - it's a PBKDF using Bcrypt as a primitive.
 */
...
(function (root, factory) {
  ...
  const { RegisterAlgorithm, CategoryType, SecurityStatus, ComplexityType, CountryCode,
          KdfAlgorithm, IKdfInstance, LinkItem, Vulnerability, KeySize } = AlgorithmFramework;

  // ===== EKSBLOWFISH (Expensive Key Schedule Blowfish) =====
  // This is the bcrypt primitive - NOT the same as standard Blowfish
  ...
  class EksBlowfish {
    ...
    expensiveKeySchedule(salt, saltLen, password, passwordLen, cost) {
      ...
      for (let round = 0; round < rounds; round++) {
        ...
        this._expandKey0(salt, saltLen);

        // Then expand with password
        this._expandKey0(password, passwordLen);
      }
    }
    ...
    encryptBlock(data, offset) {
      ...
      data[offset + 2] = leftBytes[2];
      data[offset + 3] = leftBytes[3];
      data[offset + 4] = rightBytes[0];
      data[offset + 5] = rightBytes[1];
      data[offset + 6] = rightBytes[2];
      data[offset + 7] = rightBytes[3];
    }
  }
  ...
  class BcryptPBKDFAlgorithm extends KdfAlgorithm {
    constructor() {
      super();

      // Required metadata
      this.name = "Bcrypt-PBKDF";
      this.description = "OpenBSD's password-based key derivation function using eksblowfish (expensive key schedule Blowfish). Uses Bcrypt as a cryptographic primitive with iterative hashing for strong key stretching. Different from standard Bcrypt password hashing - designed specifically for key derivation in OpenSSH and other applications.";
      this.inventor = "Niels Provos, Ted Unangst";
      this.year = 2013;
      ...
      this.documentation = [
        new LinkItem("OpenBSD bcrypt_pbkdf Source", "https://github.com/openbsd/src/blob/master/lib/libutil/bcrypt_pbkdf.c"),
        new LinkItem("Botan Bcrypt-PBKDF Implementation", "https://github.com/randombit/botan/blob/master/src/lib/pbkdf/bcrypt_pbkdf/"),
        new LinkItem("OpenSSH Usage of bcrypt_pbkdf", "https://github.com/openssh/openssh-portable/blob/master/sshkey.c")
      ];

      this.references = [
        new LinkItem("Provos-Mazieres Paper (Original Bcrypt)", "https://www.usenix.org/legacy/events/usenix99/provos/provos.pdf"),
        new LinkItem("Password Hashing Competition", "https://password-hashing.net/"),
        new LinkItem("Bcrypt vs PBKDF2 Analysis", "https://security.stackexchange.com/questions/4781/do-any-security-experts-recommend-bcrypt-for-password-storage")
      ];

      this.knownVulnerabilities = [
        new Vulnerability(
          "Computational Cost",
          "While designed to be expensive, modern GPUs can parallelize bcrypt. Use sufficient rounds (32+) for 2025."
        ),
        new Vulnerability(
          "Side-Channel Timing",
          "Implementation should use constant-time comparisons for password verification"
        )
      ];

      // Test vectors from Botan's official test suite
      this.tests = [
        {
          text: "Botan Test #1: password/salt, 12 rounds, 32 bytes",
          uri: "https://github.com/randombit/botan/blob/master/src/tests/data/bcrypt_pbkdf.vec",
          input: OpCodes.AnsiToBytes('password'),
          salt: OpCodes.Hex8ToBytes('73616C74'),
          iterations: 12,
          outputSize: 32,
          expected: OpCodes.Hex8ToBytes('1AE42C05D487BC02F64921A4EBE4EA93BCACFE135FDA99974C06B7B01FAE149A')
        },
        {
          text: "Botan Test #2: A/0001020304050607, 10 rounds, 10 bytes",
          uri: "https://github.com/randombit/botan/blob/master/src/tests/data/bcrypt_pbkdf.vec",
          input: OpCodes.AnsiToBytes('A'),
          salt: OpCodes.Hex8ToBytes('0001020304050607'),
          iterations: 10,
          outputSize: 10,
          expected: OpCodes.Hex8ToBytes('3C705DA7E10C61C2523B')
        },
        {
          text: "Botan Test #3: A/0001020304050607, 10 rounds, 64 bytes",
          uri: "https://github.com/randombit/botan/blob/master/src/tests/data/bcrypt_pbkdf.vec",
          input: OpCodes.AnsiToBytes('A'),
          salt: OpCodes.Hex8ToBytes('0001020304050607'),
          iterations: 10,
          outputSize: 64,
          expected: OpCodes.Hex8ToBytes('3CF670095D5FA753E1430C6D6177C299521B3B614D9E53FA89C7F893F33289F677DDD72EE1AA29CBF5574A948DE4623C71996FFD16C8806FDD7ACBE02992DEAC')
        },
        {
          text: "Botan Test #4: password/salt, 3 rounds, 32 bytes",
          uri: "https://github.com/randombit/botan/blob/master/src/tests/data/bcrypt_pbkdf.vec",
          input: OpCodes.AnsiToBytes('password'),
          salt: OpCodes.Hex8ToBytes('73616C74'),
          iterations: 3,
          outputSize: 32,
          expected: OpCodes.Hex8ToBytes('CCB508027535A962F591D4A7EEA08FE15EE0B68044B1A7B209605FDFBFB41228')
        },
        {
          text: "Botan Test #5: password/salt, 4 rounds, 16 bytes",
          uri: "https://github.com/randombit/botan/blob/master/src/tests/data/bcrypt_pbkdf.vec",
          input: OpCodes.AnsiToBytes('password'),
          salt: OpCodes.Hex8ToBytes('73616C74'),
          iterations: 4,
          outputSize: 16,
          expected: OpCodes.Hex8ToBytes('5BBF0CC293587F1C3635555C27796598')
        },
        {
          text: "Botan Test #6: password/salt, 8 rounds, 64 bytes",
          uri: "https://github.com/randombit/botan/blob/master/src/tests/data/bcrypt_pbkdf.vec",
          input: OpCodes.AnsiToBytes('password'),
          salt: OpCodes.Hex8ToBytes('73616C74'),
          iterations: 8,
          outputSize: 64,
          expected: OpCodes.Hex8ToBytes('E1367EC5151A33FAAC4CC1C144CD23FA15D5548493ECC99B9B5D9C0D3B27BEC76227EA66088B849B20AB7AA478010246E74BBA51723FEFA9F9474D6508845E8D')
        },
        {
          text: "Botan Test #7: password/salt, 10 rounds, 32 bytes",
          uri: "https://github.com/randombit/botan/blob/master/src/tests/data/bcrypt_pbkdf.vec",
          input: OpCodes.AnsiToBytes('password'),
          salt: OpCodes.Hex8ToBytes('73616C74'),
          iterations: 10,
          outputSize: 32,
          expected: OpCodes.Hex8ToBytes('ABC7DCFB41032D844AF67A1D1EF8A5E6A04AA3073D4F2777BFA5FE07A54CFF9D')
        }
      ];
    }
   ...
   */
    ...
    CreateInstance(isInverse = false) {
      ...
      return new BcryptPBKDFInstance(this);
    }
  }
 ...
 */
   ...
   */
    ...
    Result() {
      if (!this._inputData) {
        throw new Error('BcryptPBKDFInstance.Result: No password data fed');
      }

      if (!this.salt || this.salt.length === 0) {
        throw new Error('BcryptPBKDFInstance.Result: Salt is required');
      }
      ...
      if (this.Iterations <= 0) {
        throw new Error('BcryptPBKDFInstance.Result: Iterations must be positive');
      }

      const result = this._deriveKey(
        this._inputData,
        this.salt,
        this.Iterations,
        this.OutputSize
      ...
      OpCodes.ClearArray(this._inputData);
      this._inputData = null;
    ...
    _deriveKey(password, salt, rounds, outputLen) {
      ...
      for (let blockNum = 1; blockNum <= blocks; blockNum++) {
        ...
        // First round
        this._bcryptHash(tmp, passHash, saltHash);
        for (let i = 0; i < BCRYPT_BLOCK_SIZE; i++) {
          out[i] = tmp[i];
        }

        // Subsequent rounds (XOR with previous results)
        for (let round = 2; round <= rounds; round++) {
          // Next salt is SHA-512 of previous output
          saltHash = getSHA512Hash(tmp);
        ...
        // Copy block to output with interleaving
        for (let i = 0; i < BCRYPT_BLOCK_SIZE; i++) {
          const destIdx = i * blocks + (blockNum - 1);
          if (destIdx < outputLen) {
            output[destIdx] = out[i];
          }
        }
      }
    ...
    _bcryptHash(out, passHash, saltHash) {
      ...
      const BCRYPT_PBKDF_WORKFACTOR = 6;

      // Initialize Blowfish with salted key setup
      const bf = new EksBlowfish();
      bf.saltedSetKey(passHash, passHash.length, saltHash, saltHash.length);

      // Expensive key schedule: 64 rounds of ExpandKey
      bf.expensiveKeySchedule(saltHash, saltHash.length, passHash, passHash.length, BCRYPT_PBKDF_WORKFACTOR);

      // Copy magic to output buffer
      for (let i = 0; i < 32; i++) {
        out[i] = BCRYPT_MAGIC[i];
      }

      // Encrypt each 8-byte block 64 times
      for (let i = 0; i < 32; i += 8) {
        for (let j = 0; j < 64; j++) {
          bf.encryptBlock(out, i);
        }
      }
  ...
  RegisterAlgorithm(new BcryptPBKDFAlgorithm());

  return BcryptPBKDFAlgorithm;
}));
