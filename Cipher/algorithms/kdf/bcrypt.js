/*
 * Bcrypt Password Hashing Function Implementation
 * Production-ready implementation compatible with OpenBSD bcrypt
 * (c)2006-2025 Hawkynt
 *
 * Implements the Bcrypt password hashing function designed by Niels Provos
 * and David Mazières in 1999. Based on the Blowfish cipher with an expensive
 * key schedule (Eksblowfish) that makes brute-force attacks computationally
 * expensive. Industry standard for secure password storage.
 *
 * Reference Implementation: OpenBSD bcrypt, Botan library
 * Test Vectors: OpenBSD, Botan test suite, crypt_blowfish
 */
...
(function (root, factory) {
  ...
  const { RegisterAlgorithm, CategoryType, SecurityStatus, ComplexityType, CountryCode,
          KdfAlgorithm, IKdfInstance, TestCase, LinkItem, Vulnerability, KeySize } = AlgorithmFramework;

  // ===== BCRYPT BASE64 ENCODING (NON-STANDARD ALPHABET) =====

  // Bcrypt uses a custom base64 alphabet different from standard RFC 4648
  ...
  // ===== BCRYPT ALGORITHM CLASS =====

  class BcryptAlgorithm extends KdfAlgorithm {
    constructor() {
      super();

      // Required metadata
      this.name = "Bcrypt";
      this.description = "Industry-standard password hashing function based on Blowfish cipher with expensive key schedule. Designed by Niels Provos and David Mazières. Provides configurable work factor for future-proof security against brute-force attacks.";
      ...
      this.documentation = [
        new LinkItem("Original Paper - A Future-Adaptable Password Scheme", "https://www.usenix.org/legacy/events/usenix99/provos/provos.pdf"),
        new LinkItem("OpenBSD bcrypt Implementation", "https://cvsweb.openbsd.org/cgi-bin/cvsweb/src/lib/libc/crypt/bcrypt.c"),
        new LinkItem("Wikipedia - bcrypt", "https://en.wikipedia.org/wiki/Bcrypt"),
        new LinkItem("Botan Bcrypt Documentation", "https://botan.randombit.net/handbook/api_ref/passhash.html#bcrypt")
      ];

      this.references = [
        new LinkItem("Bcrypt Security Analysis", "https://www.usenix.org/conference/woot14/workshop-program/presentation/forler"),
      ...
      this.knownVulnerabilities = [
        new Vulnerability(
          "72-Character Truncation",
          "Bcrypt truncates passwords at 72 characters. For longer passwords, pre-hash with SHA-256 and use hex encoding."
        ),
        new Vulnerability(
          "Work Factor Evolution",
          "Cost factor should increase over time as hardware improves. Current minimum recommended: 10-12 (2023)."
        )
      ];

      // Test vectors from multiple authoritative sources
      // Format: Password (hex), Cost, Salt (16 bytes), Expected hash (23 bytes raw output)
      // Expected values are extracted from the full bcrypt hash strings (last 31 base64 chars decoded to 23 bytes)
      // Salt is exactly 16 bytes - bcrypt base64 encodes 16 bytes to 22 chars, so we decode and trim
      this.tests = [
        {
          text: "Botan Test Vector #1: 'abc' with cost 5",
        ...
        {
          text: "Botan Test Vector #2: Empty password with cost 5",
          uri: "https://github.com/randombit/botan/blob/master/src/tests/data/passhash/bcrypt.vec",
        ...
        },
        {
          ...
          uri: "https://github.com/randombit/botan/blob/master/src/tests/data/passhash/bcrypt.vec",
          input: OpCodes.Hex8ToBytes("41"), // "A"
          cost: 4,
          salt: bcryptBase64Decode("......................").slice(0, 16),
          expected: bcryptBase64Decode("1ylfG1rzUcx/p4E2WWXbK1hNBjulV/e").slice(0, 23)
        },
        {
          text: "Botan Test Vector #7: OpenBSD 'AB'",
          uri: "https://github.com/randombit/botan/blob/master/src/tests/data/passhash/bcrypt.vec",
          input: OpCodes.Hex8ToBytes("4142"), // "AB"
          cost: 4,
          salt: bcryptBase64Decode("......................").slice(0, 16),
          expected: bcryptBase64Decode("eLmsldq6Kef4lNzfsH3GLn5XEr9/dky").slice(0, 23)
        },
        {
          text: "Botan Test Vector #8: OpenBSD 'ABC'",
          uri: "https://github.com/randombit/botan/blob/master/src/tests/data/passhash/bcrypt.vec",
          input: OpCodes.Hex8ToBytes("414243"), // "ABC"
          cost: 4,
          salt: bcryptBase64Decode("......................").slice(0, 16),
          expected: bcryptBase64Decode("SeoxjjNIHEA7B01Yt2Fq2YNSKittPem").slice(0, 23)
        },
        {
          text: "Botan Test Vector #9: OpenBSD 'ABCD'",
          uri: "https://github.com/randombit/botan/blob/master/src/tests/data/passhash/bcrypt.vec",
          input: OpCodes.Hex8ToBytes("41424344"), // "ABCD"
          cost: 4,
          salt: bcryptBase64Decode("......................").slice(0, 16),
          expected: bcryptBase64Decode("5UWApRNcV8gDN6km6KdMC7MJRnFLJLi").slice(0, 23)
        },
        {
          text: "Botan Test Vector #10: OpenBSD 'ABCDE'",
          uri: "https://github.com/randombit/botan/blob/master/src/tests/data/passhash/bcrypt.vec",
          input: OpCodes.Hex8ToBytes("4142434445"), // "ABCDE"
          cost: 4,
          salt: bcryptBase64Decode("......................").slice(0, 16),
          expected: bcryptBase64Decode("aqTIOQ1wy6xo2DKiG2jtVHuLfBC.Cte").slice(0, 23)
        },
        {
          text: "Botan Test Vector #11: OpenBSD 'ABCDEF'",
          uri: "https://github.com/randombit/botan/blob/master/src/tests/data/passhash/bcrypt.vec",
          input: OpCodes.Hex8ToBytes("414243444546"), // "ABCDEF"
          cost: 4,
          salt: bcryptBase64Decode("......................").slice(0, 16),
          expected: bcryptBase64Decode("3eUc6EVdIHLPSHsfENd73y1qHUgeET6").slice(0, 23)
        },
        {
          text: "Botan Test Vector #12: OpenBSD 'ABCDEFG'",
          uri: "https://github.com/randombit/botan/blob/master/src/tests/data/passhash/bcrypt.vec",
          input: OpCodes.Hex8ToBytes("41424344454647"), // "ABCDEFG"
          cost: 4,
          salt: bcryptBase64Decode("......................").slice(0, 16),
          expected: bcryptBase64Decode("FuV659LudNIL0yJfqrb.JB0ab1eCXCy").slice(0, 23)
        },
        {
          text: "Botan Test Vector #13: OpenBSD 'ABCDEFGH'",
          uri: "https://github.com/randombit/botan/blob/master/src/tests/data/passhash/bcrypt.vec",
          input: OpCodes.Hex8ToBytes("4142434445464748"), // "ABCDEFGH"
          cost: 4,
          salt: bcryptBase64Decode("......................").slice(0, 16),
          expected: bcryptBase64Decode("1WDT31a/PBuYi4hmam2gvmgA54t9HUO").slice(0, 23)
        },
        {
          text: "Botan Test Vector #14: OpenBSD 'ABCDEFGHI'",
          uri: "https://github.com/randombit/botan/blob/master/src/tests/data/passhash/bcrypt.vec",
          input: OpCodes.Hex8ToBytes("414243444546474849"), // "ABCDEFGHI"
          cost: 4,
          salt: bcryptBase64Decode("......................").slice(0, 16),
          expected: bcryptBase64Decode("aHMfyD101pOa19Avcj8wFk7x8JyP/Oi").slice(0, 23)
        },
        {
          text: "Botan Test Vector #15: OpenBSD 'ABCDEFGHIJ'",
          uri: "https://github.com/randombit/botan/blob/master/src/tests/data/passhash/bcrypt.vec",
          input: OpCodes.Hex8ToBytes("4142434445464748494A"), // "ABCDEFGHIJ"
          cost: 4,
          salt: bcryptBase64Decode("......................").slice(0, 16),
          expected: bcryptBase64Decode("MQooKLE8.P36GbWDUbrk2NT3PYZsXOG").slice(0, 23)
        }
      ];
    }
   ...
   */
    ...
    CreateInstance(isInverse = false) {
      return new BcryptInstance(this, isInverse);
    }
  }
 ...
 */
   ...
   */
    ...
    Feed(data) {
      if (!Array.isArray(data)) {
        throw new Error('BcryptInstance.Feed: Input must be byte array (password)');
      }
   ...
   */
    ...
    bcryptHash(password, salt, cost) {
      ...
      // Return only first 23 bytes (standard bcrypt output before base64)
      // The 24th byte is dropped in the bcrypt specification
  ...
  // Return module exports (for testing)
  return {
    BcryptAlgorithm,
    BcryptInstance,
    bcryptBase64Encode,
