 ...
 * LZ77-style token stream: a per-token literal/match flag, a length code, and a
 * tiered fixed-width distance field. No official bitstream specification was
 * ever published by Microsoft (its TechNet material describes the driver's
 * behavior and MRCI hardware API only, not the codec bit layout); Microsoft's
 * compression scheme became the subject of Stac Electronics, Inc. v. Microsoft
 * Corp. (1994) because of its closeness to Stac's patented LZS algorithm.
 *
 * This is a documented-subset, from-scratch reimplementation of the general
 * technique described in secondary sources: a sliding window covering 4KB of
 * back-reference distance, a minimum match length of 2 bytes, a 2-bit length
 * class (with 6/8-bit extensions for longer matches), and a 2-bit distance
 * class selecting one of four fixed-width offset tiers. The token bit layout
 * (flag, then length, then distance; LSB-first packing) and greedy hash-chain
 * match search mirror the parameters used by CompressionWorkbench's clean-room
 * DoubleSpace building block. It is a self-consistent LZ77 coder built to
 * documented parameters, not a byte-exact clone of DBLSPACE.BIN's cluster
 * format, and does not implement any cluster/sector container framing (out of
...
(function (root, factory) {
  ...
  function writeDistance(writer, distance) {
    ...
    throw new Error('DoubleSpace: distance exceeds maximum class range');
  }
  ...
  function svdcCompress(input, maxDistance) {
  ...
  function svdcDecompress(input) {
    ...
    while (output.length < originalLength) {
      ...
      if (flag === 0) {
        ...
        if (distance < 1 || distance > output.length) throw new Error('DoubleSpace: invalid back-reference distance');
  ...
  class DoubleSpaceCompression extends CompressionAlgorithm {
    constructor() {
      ...
      this.description = "MS-DOS 6.0/6.2 real-time disk compression codec (DBLSPACE.BIN, SVDC cluster format). Sliding-window LZ77 with a 4KB window, a 2-bit length class (extending to 14 bits), and a 2-bit distance class selecting one of four fixed-width offset tiers; minimum match length 2 bytes. Documented-subset reimplementation - no official bitstream spec exists.";
  ...
  class DoubleSpaceInstance extends IAlgorithmInstance {
    ...
    Result() {
      ...
      this.inputBuffer = [];
      if (this.isInverse) return svdcDecompress(data);
      return svdcCompress(data, DS_MAX_DISTANCE);
    }
  }
