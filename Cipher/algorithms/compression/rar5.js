...
(function (root, factory) {
  ...
  // A 4-ary implicit min-heap; the tie-breaking behaviour of this heap decides
  // the shape of the Huffman tree, and therefore the emitted code lengths.
  class QuaternaryHeap {
    ...
    enqueue(element, priority) {
      ...
      while (nodeIndex > 0) {
        const parentIndex = OpCodes.Shr32Signed(nodeIndex - 1, 2);
    ...
    dequeue() {
      ...
      if (lastNodeIndex > 0) {
        let nodeIndex = 0;
        let i;
        while ((i = OpCodes.Shl32(nodeIndex, 2) + 1) < size) {
          let minChild = this.nodes[i];
          let minChildIndex = i;
          const upperBound = Math.min(i + 4, size);
          while (++i < upperBound) {
            const next = this.nodes[i];
            if (next.priority < minChild.priority) {
              minChild = next;
              minChildIndex = i;
            }
          }
          if (lastNode.priority <= minChild.priority) break;
  ...
  function buildCodeLengths(frequencies, numSymbols, maxBits) {
    ...
    const symbols = [];
    for (let i = 0; i < numSymbols; ++i)
      if (frequencies[i] > 0) symbols.push({ sym: i, freq: frequencies[i] });
    ...
    const heap = new QuaternaryHeap();
    const nodes = [];
    for (let i = 0; i < symbols.length; ++i) {
      nodes.push({ freq: symbols[i].freq, sym: symbols[i].sym, left: -1, right: -1 });
      heap.enqueue(i, symbols[i].freq);
    }
    ...
    while (heap.count > 1) {
      ...
      nodes.push({ freq: a.priority + b.priority, sym: -1, left: a.element, right: b.element });
      heap.enqueue(newIndex, a.priority + b.priority);
    }

    const root = heap.dequeue().element;

    const walk = function (index, depth) {
    ...
    constructor() {
      ...
      this.tests = [
        ...
        new TestCase(
          ...
          [
            0xb4, 0x00, 0x00, 0x00, 0xc3, 0xd9, 0x40, 0x53, 0x55, 0x53, 0x20, 0x00, 0x00, 0x00, 0x00, 0x40,
            0x42, 0x4a, 0xf9, 0x05, 0x54, 0xf6, 0x85, 0xc2, 0x14, 0x3c, 0x14, 0x3f, 0x70, 0xbf, 0xf4, 0x02,
            0x26, 0xa2, 0x36, 0x77, 0x93, 0xaa, 0xa8, 0x80, 0x9a, 0x24, 0x4f, 0xfb, 0x06, 0x8e, 0xd2, 0x3a,
            0x6e, 0x19, 0x33, 0x5e, 0x43, 0x8d, 0x86, 0xc9, 0xc7, 0x5e, 0x03, 0x46, 0xcc, 0x7c, 0xd2, 0x5e,
            0x2e, 0x35, 0x2f, 0x57, 0xf3, 0x90, 0xf0
          ],
        ...
        new TestCase(
          ...
          [
            0x0c, 0x00, 0x00, 0x00, 0xc0, 0x8f, 0x15, 0x01, 0x20, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x02, 0xdb, 0x4f, 0xfe, 0xda, 0xcc, 0x1b, 0x33, 0x06, 0x67, 0xe6, 0x80
          ],
        ...
        new TestCase(
          ...
          [
            ...
            0x22, 0x68, 0x81, 0x24, 0x08, 0xed, 0x0b, 0xc9, 0xd0, 0x23, 0x86, 0x10, 0x1c, 0xc1, 0x63, 0xc5,
            0x90, 0x12, 0x05, 0xbb, 0x86, 0x90, 0x81, 0x44, 0x0b, 0x44, 0x73, 0x08, 0x49, 0x1a, 0xf5, 0xfc,
            0xb7, 0xfe, 0x07, 0xff, 0x1f, 0x48, 0xac, 0xaa, 0xff, 0x60, 0x29, 0xc7, 0x17, 0x6d, 0xbf, 0xf8,
            0x1b, 0x66, 0x77, 0x1d, 0x00
          ],
        ...
        new TestCase(
          ...
          [
            ...
            0x42, 0x0a, 0xf8, 0x01, 0x96, 0x0a, 0xe0, 0x34, 0x55, 0x45, 0x6c, 0x5a, 0x7c, 0x55, 0xa7, 0x8f,
            0xfd, 0x6a, 0xc1, 0xd7, 0xd5, 0x8a, 0xdb, 0x01, 0xb6, 0x1f, 0xf3, 0x65, 0xa1, 0x1c, 0x6d, 0x3e,
            0xb6, 0x9c, 0x88, 0x69, 0x68, 0x6b, 0x5b, 0x07, 0x4a, 0x0d, 0x1e, 0x59, 0xaf, 0x56, 0x46, 0x2a,
            0xcf, 0x42, 0x68, 0xff, 0x2f, 0x58, 0xe4, 0xc2, 0x1c, 0x68, 0x3a, 0xce, 0x4e, 0xbb
          ],
