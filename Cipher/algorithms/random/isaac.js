/*
 * ISAAC (Indirection, Shift, Accumulate, Add, Count) Cryptographic PRNG
 * Based on BouncyCastle implementation (org.bouncycastle.crypto.engines.ISAACEngine)
 * Original design by Bob Jenkins (1996)
 * Source: https://www.burtleburtle.net/bob/rand/isaacafa.html
 *
...
(function (root, factory) {
  ...
  class IsaacAlgorithm extends RandomGenerationAlgorithm {
    constructor() {
      ...
      // Required metadata
      this.name = "ISAAC";
      this.description = "ISAAC (Indirection, Shift, Accumulate, Add, Count) is a cryptographically secure PRNG designed by Bob Jenkins. Features 8KB internal state, extremely long period (minimum 2^40, average 2^8295), and high performance.";
      ...
      this.SupportedSeedSizes = [new KeySize(0, 1024, 1)]; // Up to full state size (256 * 4 = 1024 bytes)

      // Technical specifications
      this.stateSize = STATE_ARRAY_SIZE; // 256 32-bit words
      this.keystreamSize = KEYSTREAM_SIZE; // 1024 bytes per generation cycle

      // Documentation
      this.documentation = [
        new LinkItem(
          "ISAAC Homepage - Bob Jenkins",
          "https://www.burtleburtle.net/bob/rand/isaacafa.html"
        ),
        new LinkItem(
          "ISAAC Paper - Fast Software Encryption 1996",
          "https://www.burtleburtle.net/bob/rand/isaac.html"
        ),
        new LinkItem(
          "BouncyCastle Reference Implementation",
          "https://github.com/bcgit/bc-java/blob/main/core/src/main/java/org/bouncycastle/crypto/engines/ISAACEngine.java"
        )
      ];

      this.references = [
        new LinkItem("Wikipedia: ISAAC (cipher)", "https://en.wikipedia.org/wiki/ISAAC_(cipher)"),
        new LinkItem("Bob Jenkins' Random Number Page", "https://www.burtleburtle.net/bob/rand/")
      ];

      // Official test vectors from BouncyCastle test suite
      // Source: org.bouncycastle.crypto.test.ISAACTest
      // These vectors are generated by the reference implementation
      this.tests = [
        {
          text: "BouncyCastle Test Vector #1 - Key: 00000000 (4 bytes)",
          ...
          outputSize: 2048, // Full 2048 bytes from BouncyCastle test
        ...
        {
          text: "BouncyCastle Test Vector #2 - Key: FFFFFFFF (4 bytes)",
          uri: "https://github.com/bcgit/bc-java/blob/main/core/src/test/java/org/bouncycastle/crypto/test/ISAACTest.java",
          input: null,
          seed: OpCodes.Hex8ToBytes("ffffffff"),
          outputSize: 288, // First 288 bytes from BouncyCastle outFFFFFFFF test
          expected: OpCodes.Hex8ToBytes(
        ...
        {
          text: "BouncyCastle Test Vector #3 - Key: FFFF0000 pattern (1024 bytes)",
          uri: "https://github.com/bcgit/bc-java/blob/main/core/src/test/java/org/bouncycastle/crypto/test/ISAACTest.java",
          input: null,
          seed: (function() {
            const k = new Array(256 * 4);
            for (let i = 0; i < k.length; ++i) {
              k[i] = (i % 4 === 0 || i % 4 === 1) ? 0xff : 0x00;
            }
            return k;
          })(),
          outputSize: 576, // First 576 bytes from BouncyCastle outFFFF0000 test
          expected: OpCodes.Hex8ToBytes(
        ...
        {
          text: "BouncyCastle Test Vector #4 - Key: 0000FFFF pattern (1024 bytes)",
          uri: "https://github.com/bcgit/bc-java/blob/main/core/src/test/java/org/bouncycastle/crypto/test/ISAACTest.java",
          input: null,
          seed: (function() {
            const k = new Array(256 * 4);
          ...
          outputSize: 576, // First 576 bytes from BouncyCastle out0000FFFF test
          expected: OpCodes.Hex8ToBytes(
 ...
 */
  ...
  class IsaacInstance extends IRandomGeneratorInstance {
    constructor(algorithm) {
      ...
      this.engineState = new Uint32Array(STATE_ARRAY_SIZE); // mm[] - memory
      this.results = new Uint32Array(STATE_ARRAY_SIZE);      // randrsl[] - results
      this.a = 0; // Accumulator
      this.b = 0; // Last result
      this.c = 0; // Counter

      // Keystream buffer and index
    ...
    set seed(keyBytes) {
      if (!keyBytes || keyBytes.length === 0) {
        // Initialize with zero seed
        this._initState(new Uint8Array(0));
        return;
      }

      this._initState(new Uint8Array(keyBytes));
    }
    ...
    _initState(keyBytes) {
      ...
      const abcdefgh = new Uint32Array(SIZE_L);
      abcdefgh.fill(GOLDEN_RATIO);

      // Mix the golden ratio initialization
      for (let i = 0; i < 4; ++i) {
        this._mix(abcdefgh);
      }

      // Fill engineState with mixed initialization
      // Two passes: first with results[], second with engineState[]
      for (let pass = 0; pass < 2; ++pass) {
        for (let j = 0; j < STATE_ARRAY_SIZE; j += SIZE_L) {
          ...
          // Copy mixed values to engineState
          for (let k = 0; k < SIZE_L; ++k) {
    ...
    _isaac() {
      ...
      // Convert results to big-endian keystream bytes
      // BouncyCastle uses Pack.intToBigEndian(results)
      for (let i = 0; i < STATE_ARRAY_SIZE; ++i) {
        const bytes = OpCodes.Unpack32BE(this.results[i]);
        this.keyStream[i * 4] = bytes[0];
        this.keyStream[i * 4 + 1] = bytes[1];
        this.keyStream[i * 4 + 2] = bytes[2];
        this.keyStream[i * 4 + 3] = bytes[3];
      }
    }
    ...
    NextBytes(length) {
      if (!this.initialised) {
        throw new Error("ISAAC not initialised");
      }
   ...
   */
    ...
    Result() {
      ...
      // If we have input buffer (stream cipher mode), XOR with keystream
      if (this._inputBuffer && this._inputBuffer.length > 0) {
        const output = [];
        for (let i = 0; i < this._inputBuffer.length; ++i) {
          output.push(this.returnByte(this._inputBuffer[i]));
        }
        this._inputBuffer = [];
        return output;
      }

      // Otherwise, generate random bytes
  ...
  // Register algorithm
  const algorithmInstance = new IsaacAlgorithm();
  if (!AlgorithmFramework.Find(algorithmInstance.name)) {
    RegisterAlgorithm(algorithmInstance);
  }

  return { IsaacAlgorithm, IsaacInstance };
}));
