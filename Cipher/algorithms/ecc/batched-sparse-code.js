...
(function (root, factory) {
  ...
  class BATSCodeAlgorithm extends ErrorCorrectionAlgorithm {
    constructor() {
      ...
      // Test vectors from theoretical analysis and design specifications
      this.tests = [
        new TestCase(
          [0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08], // 8 source packets
          [0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x03, 0x05, 0x07, 0x09], // Source + batch encoded
          "Single batch encoding test",
          "BATS design specification test vector"
        ),
        new TestCase(
          [0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17], // 8 source packets
          [0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x23, 0x25, 0x27, 0x29], // Source + recoding
          "Multi-batch with recoding test",
          "BATS network recoding scenario"
        ),
        new TestCase(
          [0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0x00, 0x11], // 8 source packets
          [0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0x00, 0x11, 0x55, 0x44, 0x33, 0x22], // Recovery from mixed batches
          "Recovery from mixed batches test",
          "BATS lossy network scenario"
        ),
        new TestCase(
          [0xFF, 0xFE, 0xFD, 0xFC, 0xFB, 0xFA, 0xF9, 0xF8], // 8 source packets
          [0xFF, 0xFE, 0xFD, 0xFC, 0xFB, 0xFA, 0xF9, 0xF8, 0x01, 0x02, 0x03, 0x04], // Round-trip validation
          "Round-trip encoding/decoding test",
          "BATS full cycle validation"
        )
      ];
