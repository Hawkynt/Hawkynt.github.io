...
(function (root, factory) {
  ...
  function PERM1_INNER(x) {
    ...
    const swapped = ((x & 0x0f0f) << 4) | ((x & 0xf0f0) >>> 4);
  ...
  function PERM0(x) {
    ...
    // leftRotate12_16
    return (((inner << 12) | (inner >>> 4)) & 0xFFFF);
  }
  ...
  function PERM2(x) {
    const inner = PERM1_INNER(x);
    // leftRotate4_16
    return (((inner << 4) | (inner >>> 12)) & 0xFFFF);
  }

  function PERM3(x) {
    const inner = PERM1_INNER(x);
    // leftRotate8_16
    return (((inner << 8) | (inner >>> 8)) & 0xFFFF);
  }
  ...
  function gift64tEncrypt(ks, output, input, tweak) {
    ...
    for (let round = 0; round < 28; ++round) {
      ...
      // AddRoundKey
      s0 = (s0 ^ (w3 & 0xFFFF)) & 0xFFFF;
      s1 = (s1 ^ (w3 >>> 16)) & 0xFFFF;
      s3 = (s3 ^ (0x8000 ^ GIFT64_RC[round])) & 0xFFFF;

      // AddTweak every 4 rounds except last
      if (((round + 1) % 4) === 0 && round < 27 && tweak !== 0) {
        s2 = (s2 ^ tweak) & 0xFFFF;
      }
  ...
  function gift64tDecrypt(ks, output, input, tweak) {
    ...
    w0 = (((w0 & 0xC0000000) >>> 14) | ((w0 & 0x3FFF0000) << 2) |
          ((w0 & 0x0000000F) << 12) | ((w0 & 0x0000FFF0) >>> 4)) >>> 0;
    w1 = (((w1 & 0xC0000000) >>> 14) | ((w1 & 0x3FFF0000) << 2) |
          ((w1 & 0x0000000F) << 12) | ((w1 & 0x0000FFF0) >>> 4)) >>> 0;
    w2 = (((w2 & 0xC0000000) >>> 14) | ((w2 & 0x3FFF0000) << 2) |
          ((w2 & 0x0000000F) << 12) | ((w2 & 0x0000FFF0) >>> 4)) >>> 0;
    w3 = (((w3 & 0xC0000000) >>> 14) | ((w3 & 0x3FFF0000) << 2) |
          ((w3 & 0x0000000F) << 12) | ((w3 & 0x0000FFF0) >>> 4)) >>> 0;
    ...
    function INV_PERM2(x) {
      const rotated = ((x >>> 4) | (x << 12)) & 0xFFFF;
      return INV_PERM1_INNER(rotated);
    }

    function INV_PERM3(x) {
      const rotated = ((x >>> 8) | (x << 8)) & 0xFFFF;
      return INV_PERM1_INNER(rotated);
    }
    ...
    for (let round = 28; round > 0; --round) {
      ...
      w1 = w2;
      w2 = w3;
      w3 = (((temp & 0x3FFF0000) << 2) | ((temp & 0xC0000000) >>> 14) |
            ((temp & 0x0000FFF0) >>> 4) | ((temp & 0x0000000F) << 12)) >>> 0;

      // AddTweak every 4 rounds except last
      if ((round % 4) === 0 && round !== 28 && tweak !== 0) {
        s2 = (s2 ^ tweak) & 0xFFFF;
      }
      ...
      s0 = (s0 ^ (w3 & 0xFFFF)) & 0xFFFF;
      s1 = (s1 ^ (w3 >>> 16)) & 0xFFFF;
      s3 = (s3 ^ (0x8000 ^ GIFT64_RC[round - 1])) & 0xFFFF;
  ...
  function locusMul2(ks) {
    const mask = ((ks.k[0] & 0x80000000) !== 0) ? 0x87 : 0;
    ks.k[0] = ((ks.k[0] << 1) | (ks.k[1] >>> 31)) >>> 0;
    ks.k[1] = ((ks.k[1] << 1) | (ks.k[2] >>> 31)) >>> 0;
    ks.k[2] = ((ks.k[2] << 1) | (ks.k[3] >>> 31)) >>> 0;
    ks.k[3] = ((ks.k[3] << 1) ^ mask) >>> 0;
  }
    ...
    encrypt() {
      ...
      if (plen > 0) {
        ...
        while (remaining > GIFT64_BLOCK_SIZE) {
          ...
          for (let i = 0; i < GIFT64_BLOCK_SIZE; ++i) {
            X[i] = plaintext[ptPos + i] ^ state.deltaN[i];
          }
          ...
          for (let i = 0; i < GIFT64_BLOCK_SIZE; ++i) {
            ciphertext[ctPos + i] = X[i] ^ state.deltaN[i];
          }
        ...
        if (remaining > 0) {
          ...
          for (let i = 0; i < remaining; ++i) {
            ciphertext[ctPos + i] = plaintext[ptPos + i] ^ X[i];
          }
        }
      }
    ...
    decrypt() {
      ...
      if (clen > 0) {
        ...
        while (remaining > GIFT64_BLOCK_SIZE) {
          ...
          for (let i = 0; i < GIFT64_BLOCK_SIZE; ++i) {
            X[i] = ciphertext[ctPos + i] ^ state.deltaN[i];
          }
          ...
          for (let i = 0; i < GIFT64_BLOCK_SIZE; ++i) {
            plaintext[ptPos + i] = X[i] ^ state.deltaN[i];
          }
        ...
        if (remaining > 0) {
          ...
          for (let i = 0; i < remaining; ++i) {
            plaintext[ptPos + i] = ciphertext[ctPos + i] ^ X[i];
          }
