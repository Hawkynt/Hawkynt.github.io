...
(function (root, factory) {
  ...
  class SubterraneanState {
    ...
    round() {
      ...
      this.x[1] = BDN(x3, 31,  0) ^ BDN(x4, 11,  1) ^ BDN(x4, 23,  2) ^
      ...
      this.x[2] = BDN(x7, 30,  0) ^ BDN(x0,  9,  1) ^ BDN(x0, 21,  2) ^
      ...
      this.x[3] = BDN(x3, 28,  0) ^ BDN(x4,  8,  1) ^ BDN(x4, 20,  2) ^
      ...
      this.x[4] = BDN(x7, 27,  0) ^ BDN(x0,  6,  1) ^ BDN(x0, 18,  2) ^
      ...
      this.x[5] = BDN(x3, 25,  0) ^ BDN(x4,  5,  1) ^ BDN(x4, 17,  2) ^
      ...
      this.x[6] = BDN(x7, 24,  0) ^ BDN(x0,  3,  1) ^ BDN(x0, 15,  2) ^
      ...
      this.x[7] = BDN(x3, 22,  0) ^ BDN(x4,  2,  1) ^ BDN(x4, 14,  2) ^
    ...
    absorb1(data) {
      ...
      this.x[0] = (this.x[0] ^ ((x << 1) & 0x00000002)) >>> 0;
      this.x[1] = (this.x[1] ^ (x & 0x00000008)) >>> 0;
      this.x[2] = (this.x[2] ^ 0x00000001) >>> 0; // 9th padding bit is always 1
      this.x[4] = (this.x[4] ^ (((x << 6) & 0x00000100) ^ ((x << 1) & 0x00000040))) >>> 0;
      this.x[5] = (this.x[5] ^ ((x << 15) & 0x00010000)) >>> 0;
      this.x[6] = (this.x[6] ^ ((x >>> 1) & 0x00000020)) >>> 0;
      this.x[7] = (this.x[7] ^ (((x << 21) & 0x02000000) ^ ((x << 3) & 0x00000400))) >>> 0;
    }
    ...
    absorbWord(word) {
      ...
      y = (x & 0x00080008) |
          ...
          ((x & 0x00000080) << 3) |
          ((x & 0x04000000) << 4) |
          leftRotate6(x & 0x80000004) |
          ((x & 0x00400000) << 7) |
          leftRotate12(x & 0x01000200) |
          ((x & 0x00000800) << 13) |
          ((x & 0x00000002) << 15) |
          ((x & 0x08000000) >>> 15) |
          ((x & 0x00002000) << 18) |
    ...
    blank() {
      for (let i = 0; i < 8; ++i) {
        this.round();
        this.x[0] = (this.x[0] ^ 0x02) >>> 0; // padding
      }
    }
    ...
    duplex0() {
      this.round();
      this.x[0] = (this.x[0] ^ 0x02) >>> 0; // padding for empty block
    }
   ...
   */
    ...
    _encrypt() {
      ...
      while (offset + 4 <= plaintext.length) {
        ...
        const cipherWord = (extracted ^ word) >>> 0;
        state.duplex4(word);
    ...
    _decrypt() {
      ...
      while (offset + 4 <= ciphertext.length) {
        ...
        const extracted = state.extract();
        const plainWord = (extracted ^ cipherWord) >>> 0;
