...
(function (root, factory) {
  ...
  class GASCON128Permutation {
    ...
    coreRound(roundNum) {
      // Add round constant to S[2]: ((0x0F - round) left-shift 4) OR round
      ...
      // x0 XOR-equals rightRotate19(x0) XOR rightRotate28(x0)
      ...
      // x1 XOR-equals rightRotate61(x1) XOR rightRotate38(x1)
      ...
      // x2 XOR-equals rightRotate1(x2) XOR rightRotate6(x2)
      ...
      // x3 XOR-equals rightRotate10(x3) XOR rightRotate17(x3)
      ...
      // x4 XOR-equals rightRotate7(x4) XOR rightRotate40(x4)
  ...
  class GasconPermutation {
    ...
    permute(firstRound) {
      ...
      for (let round = firstRound; round < 12; ++round) {
        ...
        x2_l = OpCodes.XorN(x2_l, OpCodes.OrN(OpCodes.Shl32(0x0F - round, 4), round));

        // Substitution layer
        x0_l = OpCodes.XorN(x0_l, x4_l); x0_h = OpCodes.XorN(x0_h, x4_h);
        x2_l = OpCodes.XorN(x2_l, x1_l); x2_h = OpCodes.XorN(x2_h, x1_h);
        x4_l = OpCodes.XorN(x4_l, x3_l); x4_h = OpCodes.XorN(x4_h, x3_h);

        const t0_l = OpCodes.ToUint32(OpCodes.AndN(~x0_l, x1_l));
        const t0_h = OpCodes.ToUint32(OpCodes.AndN(~x0_h, x1_h));
        const t1_l = OpCodes.ToUint32(OpCodes.AndN(~x1_l, x2_l));
        const t1_h = OpCodes.ToUint32(OpCodes.AndN(~x1_h, x2_h));
        const t2_l = OpCodes.ToUint32(OpCodes.AndN(~x2_l, x3_l));
        const t2_h = OpCodes.ToUint32(OpCodes.AndN(~x2_h, x3_h));
        const t3_l = OpCodes.ToUint32(OpCodes.AndN(~x3_l, x4_l));
        const t3_h = OpCodes.ToUint32(OpCodes.AndN(~x3_h, x4_h));
        const t4_l = OpCodes.ToUint32(OpCodes.AndN(~x4_l, x0_l));
        const t4_h = OpCodes.ToUint32(OpCodes.AndN(~x4_h, x0_h));

        x0_l = OpCodes.XorN(x0_l, t1_l); x0_h = OpCodes.XorN(x0_h, t1_h);
        x1_l = OpCodes.XorN(x1_l, t2_l); x1_h = OpCodes.XorN(x1_h, t2_h);
        x2_l = OpCodes.XorN(x2_l, t3_l); x2_h = OpCodes.XorN(x2_h, t3_h);
        x3_l = OpCodes.XorN(x3_l, t4_l); x3_h = OpCodes.XorN(x3_h, t4_h);
        x4_l = OpCodes.XorN(x4_l, t0_l); x4_h = OpCodes.XorN(x4_h, t0_h);

        x1_l = OpCodes.XorN(x1_l, x0_l); x1_h = OpCodes.XorN(x1_h, x0_h);
        x3_l = OpCodes.XorN(x3_l, x2_l); x3_h = OpCodes.XorN(x3_h, x2_h);
        x0_l = OpCodes.XorN(x0_l, x4_l); x0_h = OpCodes.XorN(x0_h, x4_h);
        x2_l = OpCodes.ToUint32(~x2_l);
        x2_h = OpCodes.ToUint32(~x2_h);

        // Linear diffusion layer
        let r0 = rotr64_interleaved_odd(x0_l, x0_h, 9);
        let r1 = rotr64_interleaved(x0_l, x0_h, 14);
        x0_l = OpCodes.XorN(OpCodes.XorN(x0_l, r0[0]), r1[0]);
        x0_h = OpCodes.XorN(OpCodes.XorN(x0_h, r0[1]), r1[1]);

        r0 = rotr64_interleaved_odd(x1_l, x1_h, 30);
        r1 = rotr64_interleaved(x1_l, x1_h, 19);
        x1_l = OpCodes.XorN(OpCodes.XorN(x1_l, r0[0]), r1[0]);
        x1_h = OpCodes.XorN(OpCodes.XorN(x1_h, r0[1]), r1[1]);

        r0 = rotr64_interleaved_odd(x2_l, x2_h, 0);
        r1 = rotr64_interleaved(x2_l, x2_h, 3);
        x2_l = OpCodes.XorN(OpCodes.XorN(x2_l, r0[0]), r1[0]);
        x2_h = OpCodes.XorN(OpCodes.XorN(x2_h, r0[1]), r1[1]);

        r0 = rotr64_interleaved(x3_l, x3_h, 5);
        r1 = rotr64_interleaved_odd(x3_l, x3_h, 8);
        x3_l = OpCodes.XorN(OpCodes.XorN(x3_l, r0[0]), r1[0]);
        x3_h = OpCodes.XorN(OpCodes.XorN(x3_h, r0[1]), r1[1]);

        r0 = rotr64_interleaved_odd(x4_l, x4_h, 3);
        r1 = rotr64_interleaved(x4_l, x4_h, 20);
        x4_l = OpCodes.XorN(OpCodes.XorN(x4_l, r0[0]), r1[0]);
        x4_h = OpCodes.XorN(OpCodes.XorN(x4_h, r0[1]), r1[1]);
      }
 ...
 */
   ...
   */
    ...
    _initialize() {
      const ivLow = OpCodes.ToUint32(OpCodes.AndN(this.IV, 0xFFFFFFFF));
      const ivHigh = Math.floor(this.IV / 0x100000000);

      this.perm.B[0] = OpCodes.AndN(ivLow, 0xFF);
      this.perm.B[1] = OpCodes.AndN(OpCodes.Shr32(ivLow, 8), 0xFF);
      this.perm.B[2] = OpCodes.AndN(OpCodes.Shr32(ivLow, 16), 0xFF);
      this.perm.B[3] = OpCodes.AndN(OpCodes.Shr32(ivLow, 24), 0xFF);
      this.perm.B[4] = OpCodes.AndN(ivHigh, 0xFF);
      this.perm.B[5] = OpCodes.AndN(OpCodes.Shr32(ivHigh, 8), 0xFF);
      this.perm.B[6] = OpCodes.AndN(OpCodes.Shr32(ivHigh, 16), 0xFF);
      this.perm.B[7] = OpCodes.AndN(OpCodes.Shr32(ivHigh, 24), 0xFF);
    ...
    _encrypt() {
      ...
      const tag = [];
      for (let i = 0; i < 16; ++i) {
        tag.push(OpCodes.XorN(this.perm.B[24 + i], this._key[i]));
      }
    ...
    _decrypt() {
      ...
      const expectedTag = [];
      for (let i = 0; i < 16; ++i) {
        expectedTag.push(OpCodes.XorN(this.perm.B[24 + i], this._key[i]));
      }
  ...
  const DRYDOMAIN128_PADDED = 0x0100; // 1 << 8
  const DRYDOMAIN128_FINAL = 0x0200;  // 1 << 9
  const DRYDOMAIN128_NONCE = 0x0400;  // 1 << 10
  const DRYDOMAIN128_ASSOC_DATA = 0x0800; // 2 << 10
  const DRYDOMAIN128_MESSAGE = 0x0C00; // 3 << 10
  ...
  class DrySponge128State {
    ...
    selectX(index) {
      ...
      for (let i = 0; i < 4; ++i) {
        ...
        result = OpCodes.ToUint32(OpCodes.XorN(result, OpCodes.AndN(xW[i], mask)));
      }
      return result;
    }
    ...
    mixPhase(data) {
      ...
      mixData[0] = OpCodes.OrN(data[0], OpCodes.Shl32(data[1], 8));
      mixData[1] = OpCodes.OrN(OpCodes.Shr32(data[1], 2), OpCodes.Shl32(data[2], 6));
      mixData[2] = OpCodes.OrN(OpCodes.Shr32(data[2], 4), OpCodes.Shl32(data[3], 4));
      mixData[3] = OpCodes.OrN(OpCodes.Shr32(data[3], 6), OpCodes.Shl32(data[4], 2));
      mixData[4] = OpCodes.OrN(data[5], OpCodes.Shl32(data[6], 8));
      mixData[5] = OpCodes.OrN(OpCodes.Shr32(data[6], 2), OpCodes.Shl32(data[7], 6));
      mixData[6] = OpCodes.OrN(OpCodes.Shr32(data[7], 4), OpCodes.Shl32(data[8], 4));
      mixData[7] = OpCodes.OrN(OpCodes.Shr32(data[8], 6), OpCodes.Shl32(data[9], 2));
      mixData[8] = OpCodes.OrN(data[10], OpCodes.Shl32(data[11], 8));
      mixData[9] = OpCodes.OrN(OpCodes.Shr32(data[11], 2), OpCodes.Shl32(data[12], 6));
      mixData[10] = OpCodes.OrN(OpCodes.Shr32(data[12], 4), OpCodes.Shl32(data[13], 4));
      mixData[11] = OpCodes.OrN(OpCodes.Shr32(data[13], 6), OpCodes.Shl32(data[14], 2));
      mixData[12] = OpCodes.XorN(data[15], ds);
      mixData[13] = OpCodes.Shr32(ds, 10);

      // Mix rounds 0-12 followed by coreRound(0), then final mix round 13 without coreRound
      for (let i = 0; i < 13; ++i) {
        this.mixPhaseRound(OpCodes.AndN(mixData[i], 0x3FF));
        this.c.coreRound(0);
      }
      // Final mix round (ds >> 10) without coreRound after
      this.mixPhaseRound(OpCodes.AndN(mixData[13], 0x3FF));
    }

    mixPhaseRound(data) {
      const x0 = this.selectX(OpCodes.AndN(data, 0x03));
      const x1 = this.selectX(OpCodes.AndN(OpCodes.Shr32(data, 2), 0x03));
      const x2 = this.selectX(OpCodes.AndN(OpCodes.Shr32(data, 4), 0x03));
      const x3 = this.selectX(OpCodes.AndN(OpCodes.Shr32(data, 6), 0x03));
      const x4 = this.selectX(OpCodes.AndN(OpCodes.Shr32(data, 8), 0x03));

      this.c.S[0][0] = OpCodes.ToUint32(OpCodes.XorN(this.c.S[0][0], x0));
      this.c.S[1][0] = OpCodes.ToUint32(OpCodes.XorN(this.c.S[1][0], x1));
      this.c.S[2][0] = OpCodes.ToUint32(OpCodes.XorN(this.c.S[2][0], x2));
      this.c.S[3][0] = OpCodes.ToUint32(OpCodes.XorN(this.c.S[3][0], x3));
      this.c.S[4][0] = OpCodes.ToUint32(OpCodes.XorN(this.c.S[4][0], x4));
    }
    ...
    g() {
      ...
      let w0 = OpCodes.ToUint32(OpCodes.XorN(this.c.S[0][0], this.c.S[2][1]));
      let w1 = OpCodes.ToUint32(OpCodes.XorN(this.c.S[0][1], this.c.S[3][0]));
      let w2 = OpCodes.ToUint32(OpCodes.XorN(this.c.S[1][0], this.c.S[3][1]));
      let w3 = OpCodes.ToUint32(OpCodes.XorN(this.c.S[1][1], this.c.S[2][0]));
      ...
      for (let round = 1; round < this.rounds; ++round) {
        ...
        w0 = OpCodes.ToUint32(OpCodes.XorN(this.c.S[0][0], this.c.S[2][1]));
        w1 = OpCodes.ToUint32(OpCodes.XorN(this.c.S[0][1], this.c.S[3][0]));
        w2 = OpCodes.ToUint32(OpCodes.XorN(this.c.S[1][0], this.c.S[3][1]));
        w3 = OpCodes.ToUint32(OpCodes.XorN(this.c.S[1][1], this.c.S[2][0]));
   ...
   */
    ...
    _setup(state, finalBlock) {
      ...
      for (let i = 0; i < 16; ++i) {
        ...
        if (wordOffset < 4) {
          state.x[i] = OpCodes.AndN(OpCodes.Shr32(state.c.S[stateIdx][0], wordOffset * 8), 0xFF);
        } else {
          state.x[i] = OpCodes.AndN(OpCodes.Shr32(state.c.S[stateIdx][1], (wordOffset - 4) * 8), 0xFF);
        }
      }
      ...
      for (let i = 0; i < 16; ++i) {
        ...
        const wordOffset = i % 8;
        if (wordOffset < 4) {
          const mask = ~OpCodes.Shl32(0xFF, wordOffset * 8);
          state.c.S[stateIdx][0] = OpCodes.ToUint32(OpCodes.OrN(OpCodes.AndN(state.c.S[stateIdx][0], mask), OpCodes.Shl32(this._key[i], wordOffset * 8)));
        } else {
          const mask = ~OpCodes.Shl32(0xFF, (wordOffset - 4) * 8);
          state.c.S[stateIdx][1] = OpCodes.ToUint32(OpCodes.OrN(OpCodes.AndN(state.c.S[stateIdx][1], mask), OpCodes.Shl32(this._key[i], (wordOffset - 4) * 8)));
        }
      }
      ...
      if (finalBlock) {
        state.domain = OpCodes.OrN(state.domain, DRYDOMAIN128_FINAL);
      }
    ...
    _processAD(state, ad, finalBlock) {
      ...
      if (lastBlock.length < DRYSPONGE128_RATE) {
        state.domain = OpCodes.OrN(state.domain, DRYDOMAIN128_PADDED);
      }
      ...
      if (mLen > 0) {
        ...
        while (mLen - offset > DRYSPONGE128_RATE) {
          ...
          for (let i = 0; i < DRYSPONGE128_RATE; ++i) {
            output.push(OpCodes.XorN(block[i], state.r[i]));
          }
          state.f(block, DRYSPONGE128_RATE);
          offset += DRYSPONGE128_RATE;
        }
        ...
        state.domain = OpCodes.OrN(DRYDOMAIN128_MESSAGE, DRYDOMAIN128_FINAL);
        if (lastBlock.length < DRYSPONGE128_RATE) {
          state.domain = OpCodes.OrN(state.domain, DRYDOMAIN128_PADDED);
        }

        for (let i = 0; i < lastBlock.length; ++i) {
          output.push(OpCodes.XorN(lastBlock[i], state.r[i]));
        }
    ...
    _decrypt() {
      ...
      if (ctLen > 0) {
        ...
        while (ctLen - offset > DRYSPONGE128_RATE) {
          ...
          for (let i = 0; i < DRYSPONGE128_RATE; ++i) {
            plainBlock[i] = OpCodes.XorN(block[i], state.r[i]);
        ...
        for (let i = 0; i < lastBlockLen; ++i) {
          plainBlock[i] = OpCodes.XorN(lastBlock[i], state.r[i]);
          output.push(plainBlock[i]);
        }
      ...
      let tagMatch = 0;
      for (let i = 0; i < DRYGASCON128_TAG_SIZE; ++i) {
        tagMatch = OpCodes.OrN(tagMatch, OpCodes.XorN(receivedTag[i], computedTag[i]));
      }
