/*
 * KNOT-AEAD-128-384 - NIST Lightweight Cryptography Finalist
 * AlgorithmFramework Implementation
 * (c)2006-2025 Hawkynt
 *
 * KNOT-AEAD-128-384 is an authenticated encryption algorithm based on the
 * KNOT permutation family. It uses a 384-bit sponge construction with a
 * 128-bit key, 192-bit nonce (though only 128 bits used), and produces a
 * 128-bit authentication tag. Compared to KNOT-128-256, it offers higher
 * throughput with a 192-bit rate (24 bytes).
 *
 * Features:
 * - 128-bit key, 128-bit nonce
 * - 128-bit authentication tag
 * - 384-bit state with 192-bit rate (24 bytes)
 * - Bit-sliced S-box with linear diffusion
 * - NIST LWC finalist
 *
 * Permutation Structure:
 * - State: 4 rows × 96 bits = 384 bits total
 *   - Rows 0,2,4,6: 64-bit words
 *   - Rows 1,3,5,7: 32-bit words
 * - S-box: 4-bit nonlinear substitution applied in bit-sliced mode
 * - Linear layer: Row rotations (1, 8, 55 bits)
 * - Round constants: 7-bit LFSR-based (104 rounds max)
 *
 * References:
...
(function (root, factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    root.KnotAead128384 = factory(root.AlgorithmFramework, root.OpCodes);
  }
}((function() {
  ...
  /**
   * Pack 8 bytes (little-endian) into JavaScript number (53-bit safe integer)
   * Uses only lower 6 bytes to stay within safe integer range
   */
  function bytesToInt64(bytes) {
    // JavaScript can safely represent 2^53 - 1, so we use 6 bytes max
    let result = 0;
    for (let i = 0; i < 6 && i < bytes.length; ++i) {
      result |= (bytes[i] & 0xFF) * Math.pow(2, i * 8);
    }
    // Handle bytes 6-7 carefully
    if (bytes.length > 6) {
      result += (bytes[6] & 0xFF) * 0x1000000000000; // 2^48
    }
    if (bytes.length > 7) {
      result += (bytes[7] & 0xFF) * 0x100000000000000; // 2^56
    }
    return result;
  }

  /**
   * Pack 4 bytes (little-endian) into 32-bit unsigned integer
   */
   ...
   * Note: JavaScript loses precision above 2^53, but we handle it with modulo
   */
  function int64ToBytes(value) {
    const bytes = new Array(8);
    // Use modulo to handle values beyond 2^53
    for (let i = 0; i < 8; ++i) {
      bytes[i] = Math.floor(value / Math.pow(2, i * 8)) & 0xFF;
    }
    return bytes;
  }

  /**
   * Unpack 32-bit value to 4 bytes (little-endian)
   */
   ...
   * @param {number} bits - Rotation amount (0-31)
   * @returns {Object} {low64, high32} - Rotated byte arrays
   */
  function rotL96Short(low64bytes, high32bytes, bits) {
    if (bits === 0) {
      return { low64: [...low64bytes], high32: [...high32bytes] };
    }

    // Work at the bit level using BigInt for exact 96-bit arithmetic
    // Combine into 96-bit value
    let value = 0n;
    for (let i = 0; i < 8; ++i) {
  ...
  /**
   * Left rotate 96-bit value (represented as 64-bit low + 32-bit high)
   * Long rotation (positions >= 32)
   * Formula from C reference:
   *   a0 = (b0 << bits) | (((uint64_t)b1) << (bits - 32)) | (b0 >> (96 - bits))
   *   a1 = (uint32_t)((b0 << (bits - 32)) >> 32)
   *
   * @param {Array} low64bytes - 8-byte array (low 64 bits, little-endian)
   * @param {Array} high32bytes - 4-byte array (high 32 bits, little-endian)
   * @param {number} bits - Rotation amount (32-95)
   * @returns {Object} {low64, high32} - Rotated byte arrays
   */
  function rotL96Long(low64bytes, high32bytes, bits) {
  ...
  function knot384Permute(state, rounds) {
    ...
    for (let i = 0; i < rounds; ++i) {
      ...
      const rot1 = rotL96Short(b2, b3, 1);
      x2 = rot1.low64;
      x3 = rot1.high32;

      // Row 2: rotate by 8 bits
      const rot8 = rotL96Short(b4, b5, 8);
      x4 = rot8.low64;
      x5 = rot8.high32;

      // Row 3: rotate by 55 bits
      const rot55 = rotL96Long(b6, b7, 55);
      x6 = rot55.low64;
      x7 = rot55.high32;
    }
  ...
  return KnotAead128384Algorithm;
}));
