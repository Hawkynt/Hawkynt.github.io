/*
 * ASCON-128a AEAD - NIST Lightweight Cryptography Standard (High-Speed Variant)
 * Professional implementation following NIST SP 800-232 specification
 * (c)2006-2025 Hawkynt
 *
 * ASCON-128a is the high-speed variant of ASCON-128 with higher rate (128-bit vs 64-bit).
 * ASCON-128 and ASCON-128a were selected as NIST's lightweight cryptography standard.
 * This variant trades slightly reduced security margin for ~2x throughput improvement.
 *
 * Specifications:
 * - 128-bit keys
 * - 128-bit nonces
 * - 128-bit authentication tags
 * - 16-byte rate (128-bit) for associated data and plaintext (vs 8-byte for ASCON-128)
 * - 12 rounds for initialization/finalization (same as ASCON-128)
 * - 8 rounds for intermediate operations (vs 6 rounds for ASCON-128)
...
(function (root, factory) {
  ...
  // ASCON-128a initialization vector (IV)
  // Format: k=128, r=128, a=8, b=6 encoded as 0x80800c0800000000
  const ASCON128a_IV = [0x00000000, 0x80800c08]; // [low32, high32] in big-endian
  ...
  class AsconState {
     ...
     * @param {number} rounds - Number of rounds (12 for ASCON-128a init/final, 8 for intermediate)
     */
    permute(rounds) {
    ...
    /**
     * XOR byte array into state (up to 16 bytes for ASCON-128a)
     * ASCON-128a has 16-byte rate, so we XOR into x0 and x1
     */
    xorBytes(bytes, offset, length) {
      const blockOffset = offset || 0;
      const blockLength = length || bytes.length;

      // XOR into x0 (first 8 bytes)
      const block0 = new Array(8);
      for (let i = 0; i < 8; ++i) {
        if (i < blockLength) {
          block0[i] = bytes[blockOffset + i];
        } else {
          block0[i] = 0;
        }
      }
      const word0 = pack64BE(block0, 0);
      this.xorWord(0, word0);

      // XOR into x1 (next 8 bytes)
      if (blockLength > 8) {
        const block1 = new Array(8);
        for (let i = 0; i < 8; ++i) {
          if (i + 8 < blockLength) {
            block1[i] = bytes[blockOffset + 8 + i];
          } else {
            block1[i] = 0;
          }
        }
        const word1 = pack64BE(block1, 0);
        this.xorWord(1, word1);
      }
    }

    /**
     * Extract bytes from state (from x0 and x1 for encryption/decryption)
     * ASCON-128a has 16-byte rate
     */
    extractBytes(length) {
      const word0 = unpack64BE(this.x[0]);
      if (length <= 8) {
        return word0.slice(0, length);
      }
      const word1 = unpack64BE(this.x[1]);
      return word0.concat(word1.slice(0, length - 8));
    }
  ...
  // ========================[ ASCON-128a AEAD INSTANCE ]========================

  class Ascon128aInstance extends IAeadInstance {
    ...
    /**
     * Initialize ASCON-128a state with IV, Key, and Nonce
     * Uses 12-round permutation (same as ASCON-128)
     */
    initialize() {
      ...
      // x0 = IV
      this.state.setWord(0, ASCON128a_IV);
      ...
      // Apply 12-round permutation (same as ASCON-128)
     ...
     * ASCON-128a uses 16-byte rate and 8-round permutation (vs 6 rounds for ASCON-128)
     */
    processAAD() {
      if (!this.initialized) {
        this.initialize();
      ...
      if (aadLen > 0) {
        ...
        let offset = 0;
        while (offset + 16 <= aadLen) {
          this.state.xorBytes(aad, offset, 16);
          this.state.permute(8); // ASCON-128a uses 8 rounds
          offset += 16;
        }

        // Process final block (may be partial)
        const remaining = aadLen - offset;
        const finalBlock = new Array(16);
        for (let i = 0; i < 16; ++i) {
          if (i < remaining) {
        ...
        this.state.xorBytes(finalBlock, 0, 16);
        this.state.permute(8); // ASCON-128a uses 8 rounds
      }
    ...
    Result() {
      ...
      if (this.isInverse) {
        ...
        // Decrypt ciphertext - ASCON-128a uses 16-byte rate
        let offset = 0;

        // Process complete 16-byte blocks
        while (offset + 16 <= ciphertextLen) {
          const block = ciphertext.slice(offset, offset + 16);
          const keystream = this.state.extractBytes(16);
          ...
          this.state.xorBytes(block, 0, 16);
          this.state.permute(8); // ASCON-128a uses 8 rounds

          offset += 16;
        }

        // Process final block (may be 0-15 bytes)
        ...
        const paddedBlock = new Array(16);
        for (let i = 0; i < 16; ++i) {
          if (i === remaining) {
        ...
        this.state.xorBytes(paddedBlock, 0, 16);

        // Finalization: x2 ^= K[0..7], x3 ^= K[8..15] (different from ASCON-128!)
        // C reference line 539: lw_xor_block(state.B + 16, k, ASCON128_KEY_SIZE)
        // This XORs key into x2 and x3 (bytes 16-31)
        this.state.xorWord(2, pack64BE(this._key, 0));
        this.state.xorWord(3, pack64BE(this._key, 8));

        // Apply 12-round permutation for finalization (same as ASCON-128)
        this.state.permute(12);
        ...
        // Encrypt plaintext - ASCON-128a uses 16-byte rate
        let offset = 0;

        // Process complete 16-byte blocks
        while (offset + 16 <= inputLen) {
          ...
          const cipherBlock = this.state.extractBytes(16);
        ...
        // Finalization: x2 ^= K[0..7], x3 ^= K[8..15] (different from ASCON-128!)
        // C reference line 485: lw_xor_block(state.B + 16, k, ASCON128_KEY_SIZE)
        this.state.xorWord(2, pack64BE(this._key, 0));
        this.state.xorWord(3, pack64BE(this._key, 8));

        // Apply 12-round permutation for finalization (same as ASCON-128)
        // C reference line 486: ascon_permute(&state, 0) = 12 rounds
        this.state.permute(12);
  ...
  // ========================[ ASCON-128a ALGORITHM ]========================

  class Ascon128aAlgorithm extends AeadAlgorithm {
    constructor() {
      super();

      // Metadata
      this.name = "ASCON-128a";
      this.description = "High-speed variant of ASCON with 128-bit rate and 8-round permutation. Part of NIST's lightweight cryptography standard (SP 800-232).";
      ...
      // Source: Reference Sources/c-cpp-source/academic/lightweight-crypto/test/kat/ASCON-128a.txt
      this.tests = [
        {
        ...
        },
        {
          ...
          expected: OpCodes.Hex8ToBytes("6E652B55BFDC8CAD2EC43815B1666B1A3A")
        },
        {
          text: "NIST LWC KAT Count=50 (1-byte plaintext, 16-byte AAD)",
          uri: "https://csrc.nist.gov/projects/lightweight-cryptography/finalists",
          key: OpCodes.Hex8ToBytes("000102030405060708090A0B0C0D0E0F"),
          nonce: OpCodes.Hex8ToBytes("000102030405060708090A0B0C0D0E0F"),
          aad: OpCodes.Hex8ToBytes("000102030405060708090A0B0C0D0E0F"),
          input: OpCodes.Hex8ToBytes("00"),
          expected: OpCodes.Hex8ToBytes("52CDCC8F91E862ACACC86437163F8D31D6")
        }
      ];
    }

    CreateInstance(isInverse) {
      return new Ascon128aInstance(this, isInverse || false);
    }
  }

  // Register algorithm
  RegisterAlgorithm(new Ascon128aAlgorithm());

  return { Ascon128aAlgorithm, Ascon128aInstance };
}));
