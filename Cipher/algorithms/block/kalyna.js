#!/usr/bin/env node
/*
 * Kalyna Block Cipher - Universal Implementation
...
(function(global) {
  ...
  const Kalyna = {
    ...
    BLOCK_128: 16,  // 128 bits = 16 bytes
    BLOCK_256: 32,  // 256 bits = 32 bytes  
    BLOCK_512: 64,  // 512 bits = 64 bytes
     ...
     */
    Init: function(blockBits = 128, keyBits = 128) {
     ...
     * Encrypt a block
     */
    EncryptBlock: function(plaintext, key) {
     ...
     * Decrypt a block
     */
    DecryptBlock: function(ciphertext, key) {
      // Initialize if needed
      if (!this.roundKeys) {
        this.KeySchedule(key);
      }
    ...
    TestVectors: [
      {
        ...
        description: "Kalyna-128/128 test vector"
      }
    ]
  };
  ...
  if (typeof Cipher !== 'undefined' && Cipher.AddCipher) {
    const KalynaCipher = {
      ...
      Init: function() {
        Kalyna.Init(this.currentBlockSize, this.currentKeySize);
        this.currentKey = new Array(this.currentKeySize / 8).fill(0);
        return 0;
      },
      
      KeySetup: function(nKeyIndex, key) {
        try {
          const expectedLength = (this.currentKeySize / 8) * 2; // Hex chars
          if (key.length !== expectedLength) {
            return `Key must be ${expectedLength} hex characters (${this.currentKeySize} bits)`;
          }
          this.currentKey = OpCodes.HexToBytes(key);
          Kalyna.KeySchedule(this.currentKey);
      ...
      ClearData: function() {
        ...
        if (Kalyna.roundKeys) {
          for (let i = 0; i < Kalyna.roundKeys.length; i++) {
            OpCodes.ClearArray(Kalyna.roundKeys[i]);
          }
          Kalyna.roundKeys = null;
        }
      }
    };
    
    Cipher.AddCipher(Kalyna);
  }
  
  // Export for Node.js
  if (typeof module !== 'undefined' && module.exports) {
    module.exports = Kalyna;
  }
  
  // Export to global scope
  global.Kalyna = Kalyna;
