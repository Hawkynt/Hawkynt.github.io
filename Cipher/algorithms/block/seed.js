 ...
 * 
 * This implementation uses the authoritative Bouncy Castle reference for production-grade accuracy.
 */
...
(function (root, factory) {
  ...
  // SEED S-boxes - Four 32-bit S-boxes with 256 entries each (from Bouncy Castle reference)
  ...
  function createWorkingKey(keyBytes) {
    ...
    // Convert key bytes to 32-bit words using OpCodes
    let key0 = OpCodes.Pack32BE(keyBytes[0], keyBytes[1], keyBytes[2], keyBytes[3]);
    let key1 = OpCodes.Pack32BE(keyBytes[4], keyBytes[5], keyBytes[6], keyBytes[7]);
    let key2 = OpCodes.Pack32BE(keyBytes[8], keyBytes[9], keyBytes[10], keyBytes[11]);
  ...
  /**
 * SeedAlgorithm - Block cipher implementation
 * @class
 ...
 */
      ...
      this.tests = [
        {
          ...
          expected: OpCodes.Hex8ToBytes("5ebac6e0054e166819aff1cc6d346cdb")
        },
        {
          ...
          expected: OpCodes.Hex8ToBytes("c11f22f20140505084483597e4370f43")
        },
        {
  ...
  /**
 * Seed cipher instance implementing Feed/Result pattern
 * @class
 ...
 */
   ...
   */
    ...
    set key(keyBytes) {
      ...
      this.workingKey = createWorkingKey(keyBytes);
    }
   ...
   */
    ...
    _encryptBlock(input) {
      ...
      // Convert input to two 64-bit halves as [high32, low32] pairs
      let lHigh = OpCodes.Pack32BE(input[0], input[1], input[2], input[3]);
      let lLow = OpCodes.Pack32BE(input[4], input[5], input[6], input[7]);
      let rHigh = OpCodes.Pack32BE(input[8], input[9], input[10], input[11]);
      let rLow = OpCodes.Pack32BE(input[12], input[13], input[14], input[15]);
      ...
      const result1 = OpCodes.Unpack32BE(rHigh);
      const result2 = OpCodes.Unpack32BE(rLow);
      const result3 = OpCodes.Unpack32BE(lHigh);
      const result4 = OpCodes.Unpack32BE(lLow);
