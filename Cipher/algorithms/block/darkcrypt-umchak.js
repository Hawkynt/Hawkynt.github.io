/*
 * Umchak (DarkCrypt) Implementation
 * Compatible with AlgorithmFramework
 * (c)2006-2025 Hawkynt
 *
 * A TEA-family block cipher from the DarkCrypt Total Commander plugin (Alexander
 * Myasnikov, "Zarya" project). 64-bit block (2x32-bit words L,R), 512-bit key
 ...
 *      as 64 little-endian dwords, is the round-key array K[0..63] used by
 *      crypt()/decrypt().
 * crypt() runs 15 unrolled groups of 4 ARX sub-rounds over (L,R) using K[2..61],
 * with an initial K[0]/K[1] addition and a final K[62]/K[63] XOR whitening.
 * Each sub-round adds f(other), XORs in (other ^ roundConstant), then rotates by
 * either the low 5 bits or the top 5 bits of a lane (alternating low/low/top/top
 * across the 4 sub-rounds), before adding the next round key. decrypt() runs the
 * exact algebraic inverse. Test vectors verified against the DarkCrypt
 * implementation for all three reference vectors, including round-trip.
 *
 * The embedded S-box is the first 8192 bytes (32 rows x 256 cols) of the shared
 * DarkCrypt "GTEA-family" substitution table also used (in full, 128 rows) by
 * the DarkCrypt GTEA cipher; verified byte-identical against the darkcrypt-gtea.js
 * table.
 * Educational only.
 */
...
(function (root, factory) {
  ...
  // 32 rows x 256 columns substitution table, as implemented in the DarkCrypt
  // Total Commander plugin; identical to the first 8192 bytes of the larger
  // 128x256 table used by the DarkCrypt GTEA cipher.
  ...
  class DarkCryptUmchakAlgorithm extends BlockCipherAlgorithm {
    constructor() {
      ...
      this.name = "Umchak (DarkCrypt)";
      this.description = "TEA-family block cipher from the DarkCrypt Total Commander plugin: 64-bit block, 512-bit key, 64-word round-key schedule built by an RC5-constant-driven S-box mixing pass followed by a modified RC4-style permutation.";
      ...
      this.knownVulnerabilities = [
        new Vulnerability("Unanalyzed non-standard cipher", "Undocumented DarkCrypt-specific design with no public cryptanalysis.", "Use AES or another vetted cipher.")
      ];

      // Test vectors generated from the DarkCrypt implementation (raw primitive: setup(key)+crypt(block)).
      this.tests = [
        {
          text: "DarkCrypt Umchak — zero key/plaintext",
          ...
          expected: OpCodes.Hex8ToBytes("f6d28498b21e3b96")
        },
        {
          text: "DarkCrypt Umchak — incrementing key/plaintext",
          uri: "https://totalcmd.net/plugring/darkcrypttc.html",
          ...
          expected: OpCodes.Hex8ToBytes("a9c7de5d888323cb")
        },
        {
          text: "DarkCrypt Umchak — shifted incrementing key/plaintext",
          uri: "https://totalcmd.net/plugring/darkcrypttc.html",
          ...
          expected: OpCodes.Hex8ToBytes("567f7aa3640a8177")
        }
      ];
    }

    CreateInstance(isInverse = false) {
      return new DarkCryptUmchakInstance(this, isInverse);
    }
  }

  class DarkCryptUmchakInstance extends IBlockCipherInstance {
    ...
    set key(keyBytes) {
      ...
      if (keyBytes.length !== 64)
        throw new Error(`Invalid key size: ${keyBytes.length} bytes. Umchak (DarkCrypt) requires exactly 64 bytes`);
      this._key = [...keyBytes];
      this.KeySize = keyBytes.length;
      this._roundKeys = this._setup(this._key);
    }
    ...
    _setup(keyBytes) {
      ...
      for (let i = 0; i < 64; i++) {
        ...
        const b0 = OpCodes.And32(OpCodes.Shr32(ecxv, 24), 0xFF);
        const b1 = OpCodes.And32(OpCodes.Shr32(ecxv, 16), 0xFF);
        const b2 = OpCodes.And32(OpCodes.Shr32(ecxv, 8), 0xFF);
        const b3 = OpCodes.And32(ecxv, 0xFF);
        const rowEcx = OpCodes.And32(ecxv, 0x1F);
        const rowEdx = OpCodes.And32(edx, 0x1F);

        const v0 = sboxAt(rowI, b0);
        ...
        let edi = OpCodes.RotL32(K1, 16);
        const fK1 = OpCodes.Xor32(OpCodes.Shl32(K1, 6), OpCodes.Shr32(K1, 8));
        edi = OpCodes.ToUint32(edi + fK1);
        edi = OpCodes.ToUint32(edi + kwi);

        const c0 = OpCodes.And32(OpCodes.Shr32(edi, 24), 0xFF);
        const c1 = OpCodes.And32(OpCodes.Shr32(edi, 16), 0xFF);
        const c2 = OpCodes.And32(OpCodes.Shr32(edi, 8), 0xFF);
        const c3 = OpCodes.And32(edi, 0xFF);
        const rowEdx2 = OpCodes.And32(edx, 0x1F);
        const rowEdiFinal = OpCodes.And32(edi, 0x1F);

        const w0 = sboxAt(rowEdx2, c0);
    ...
    _encryptBlock(block) {
      ...
      let L = OpCodes.ToUint32(K[0] + OpCodes.Pack32LE(block[0], block[1], block[2], block[3]));
      let R = OpCodes.ToUint32(K[1] + OpCodes.Pack32LE(block[4], block[5], block[6], block[7]));

      let idx = 2;
      while (idx !== 62) {
        // sub-round 1: new L, rotate by low 5 bits of R
        L = OpCodes.ToUint32(L + this._f(R));
        L = OpCodes.ToUint32(L + OpCodes.Xor32(R, idx));
        L = OpCodes.RotL32(L, OpCodes.And32(R, 0x1F));
        L = OpCodes.ToUint32(L + K[idx]);

        // sub-round 2: new R, rotate by low 5 bits of new L
        let Rn = OpCodes.ToUint32(this._f(L) + R);
        Rn = OpCodes.ToUint32(Rn + OpCodes.Xor32(L, idx + 1));
        Rn = OpCodes.RotL32(Rn, OpCodes.And32(L, 0x1F));
        Rn = OpCodes.ToUint32(Rn + K[idx + 1]);
        R = Rn;

        // sub-round 3: new L2, rotate by top 5 bits of R
        let L2 = OpCodes.ToUint32(L + this._f(R));
        L2 = OpCodes.ToUint32(L2 + OpCodes.Xor32(R, idx + 2));
        L2 = OpCodes.RotL32(L2, OpCodes.Shr32(R, 27));
        L2 = OpCodes.ToUint32(L2 + K[idx + 2]);
        L = L2;

        // sub-round 4: new R2, rotate by top 5 bits of new L2
        let R2 = OpCodes.ToUint32(R + this._f(L));
        R2 = OpCodes.ToUint32(R2 + OpCodes.Xor32(L, idx + 3));
        R2 = OpCodes.RotL32(R2, OpCodes.Shr32(L, 27));
        R2 = OpCodes.ToUint32(R2 + K[idx + 3]);
        R = R2;

        idx += 4;
      }
      L = OpCodes.Xor32(L, K[idx]);
      R = OpCodes.Xor32(R, K[idx + 1]);
    ...
    _decryptBlock(block) {
      ...
      let idx = 61;
      R = OpCodes.Xor32(R, K[63]);
      L = OpCodes.Xor32(L, K[62]);

      while (idx !== 1) {
        // undo sub-round 4
        R = OpCodes.ToUint32(R - K[idx]);
        R = OpCodes.RotR32(R, OpCodes.Shr32(L, 27));
        R = OpCodes.ToUint32(R - this._f(L));
        R = OpCodes.ToUint32(R - OpCodes.Xor32(L, idx));

        // undo sub-round 3
        L = OpCodes.ToUint32(L - K[idx - 1]);
        L = OpCodes.RotR32(L, OpCodes.Shr32(R, 27));
        L = OpCodes.ToUint32(L - this._f(R));
        L = OpCodes.ToUint32(L - OpCodes.Xor32(R, idx - 1));

        // undo sub-round 2
        R = OpCodes.ToUint32(R - K[idx - 2]);
        R = OpCodes.RotR32(R, OpCodes.And32(L, 0x1F));
        R = OpCodes.ToUint32(R - this._f(L));
        R = OpCodes.ToUint32(R - OpCodes.Xor32(L, idx - 2));

        // undo sub-round 1
        L = OpCodes.ToUint32(L - K[idx - 3]);
        L = OpCodes.RotR32(L, OpCodes.And32(R, 0x1F));
        L = OpCodes.ToUint32(L - this._f(R));
        L = OpCodes.ToUint32(L - OpCodes.Xor32(R, idx - 3));

        idx -= 4;
      }
      L = OpCodes.ToUint32(L - K[0]);
      R = OpCodes.ToUint32(R - K[1]);
      return [...OpCodes.Unpack32LE(L), ...OpCodes.Unpack32LE(R)];
    }
  }

  const algorithmInstance = new DarkCryptUmchakAlgorithm();
  if (!AlgorithmFramework.Find(algorithmInstance.name)) {
    RegisterAlgorithm(algorithmInstance);
  }

  return { DarkCryptUmchakAlgorithm, DarkCryptUmchakInstance };
}));
