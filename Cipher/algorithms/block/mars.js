...
let MARSAlgorithm, MARSInstance;
  ...
  _expandKey(keyBytes) {
    ...
    const keyLen32 = Math.floor(keyBytes.length / 4);
    for (let i = 0; i < keyLen32; i++) {
      T[i] = OpCodes.Pack32LE(keyBytes[i*4], keyBytes[i*4+1], keyBytes[i*4+2], keyBytes[i*4+3]);
    }
    T[keyLen32] = keyLen32; // Store number of key words, not byte length

    // Linear transformation and stirring (4 iterations, each computes 10 words)
    for (let j = 0; j < 4; j++) {
  ...
  // ===== REGISTRATION =====
    ...
    const algorithmInstance = new MARSAlgorithm();
  ...
  return { MARSAlgorithm, MARSInstance };
}));