/*
 * MARS-256 (DarkCrypt) Implementation
 ...
 *
 * This variant extends the key to a 256-bit (32-byte) key (standard MARS caps at 448 bits / 56 bytes, so this is within the official range but not one of its usual named sizes).
 *
 ...
 * seeded from the key words via modulo indexing) to Nk=8 words (32-byte /
 * 256-bit key) with no upper bound enforced, and applies the standard
 * multiplication-keyword mask tweak unchanged.
 *
 * Test vectors verified against the DarkCrypt implementation, including
 * encrypt/decrypt round-trip. 256-bit key, 128-bit block. Educational only.
 */
...
(function (root, factory) {
  ...
  class DarkCryptMARS256Algorithm extends BlockCipherAlgorithm {
    constructor() {
      super();

      this.name = "MARS-256 (DarkCrypt)";
      this.description = "MARS variant from the DarkCrypt Total Commander plugin: standard MARS round structure (8 forward mixing + 16 keyed core + 8 backward mixing rounds) with the key expansion generalized to a 32-byte (256-bit) key via modulo indexing into the key-word array, beyond the official spec's 56-byte maximum.";
      ...
      this.knownVulnerabilities = [
        new Vulnerability("Non-standard key size", "256-bit key size is within the official MARS range but the specific key-expansion generalization used here (arbitrary Nk via modulo indexing) has not been independently analyzed for this DarkCrypt variant family.", "Use AES or another vetted cipher.")
      ];
      ...
      this.tests = [
        {
          ...
          expected: OpCodes.Hex8ToBytes("c90680f1f61e6f4ea42aada268a97c7a")
        },
        {
          text: "DarkCrypt Mars — incrementing key/plaintext",
          uri: "https://totalcmd.net/plugring/darkcrypttc.html",
          input: OpCodes.Hex8ToBytes("000102030405060708090a0b0c0d0e0f"),
          key: OpCodes.Hex8ToBytes("000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f"),
          expected: OpCodes.Hex8ToBytes("6b1c86d4580004b4fe3ff0bb57fc5275")
        },
        {
          text: "DarkCrypt Mars — shifted incrementing key/plaintext",
          uri: "https://totalcmd.net/plugring/darkcrypttc.html",
          input: OpCodes.Hex8ToBytes("101112131415161718191a1b1c1d1e1f"),
          key: OpCodes.Hex8ToBytes("0102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f20"),
          expected: OpCodes.Hex8ToBytes("ecf145c4976643e8dc7aa94f41879a2b")
        }
      ];
    }
  ...
  class DarkCryptMARS256Instance extends IBlockCipherInstance {
    ...
    set key(keyBytes) {
      ...
      if (keyBytes.length !== 32)
        throw new Error(`Invalid key size: ${keyBytes.length} bytes. MARS-256 (DarkCrypt) requires exactly 32 bytes`);
      this._key = [...keyBytes];
  ...
  const algorithmInstance = new DarkCryptMARS256Algorithm();
  if (!AlgorithmFramework.Find(algorithmInstance.name)) {
    RegisterAlgorithm(algorithmInstance);
  }

  return { DarkCryptMARS256Algorithm, DarkCryptMARS256Instance };
}));
