#!/usr/bin/env node
/*
 * Universal Camellia Cipher Implementation
 ...
 * Designed by NTT and Mitsubishi Electric Corporation
 * 
 * Key features:
 * - 18 rounds for 128-bit keys, 24 rounds for 192/256-bit keys
 * - FL and FL^-1 functions applied every 6 rounds
 * - S-boxes based on invertible transformation
...
(function(global) {
  ...
  if (!global.OpCodes) {
    if (typeof require !== 'undefined') {
      ...
      console.error('Camellia cipher requires OpCodes library to be loaded first');
      return;
    }
  }
  ...
  if (!global.Cipher) {
    if (typeof require !== 'undefined') {
      ...
      console.error('Camellia cipher requires Cipher system to be loaded first');
      return;
    }
  }
  
  // Create Camellia cipher object
  const Camellia = {
    // Public interface properties
    internalName: 'Camellia',
    name: 'Camellia (128/192/256-bit)',
    comment: 'Camellia cipher by NTT/Mitsubishi - 128-bit blocks, 18/24 rounds, FL functions (RFC 3713)',
    minKeyLength: 16,    // 128-bit key
    maxKeyLength: 32,    // 256-bit key
    stepKeyLength: 8,    // Support 16, 24, 32 byte keys
    ...
    SIGMA1L: 0xA09E667F, SIGMA1R: 0x3BCC908B,
    SIGMA2L: 0xB67AE858, SIGMA2R: 0x4CAA73B2,
    SIGMA3L: 0xC6EF372F, SIGMA3R: 0xE94F82BE,
    SIGMA4L: 0x54FF53A5, SIGMA4R: 0xF1D36F1C,
    SIGMA5L: 0x10E527FA, SIGMA5R: 0xDE682D1D,
    SIGMA6L: 0xB05688C2, SIGMA6R: 0xB3E6C1FD,
    ...
    // Official test vectors from RFC 3713
    testVectors: [
      { 
    ...
    ClearData: function(id) {
      if (Camellia.instances[id]) {
        ...
        if (instance.kw) {
          global.OpCodes.ClearArray(instance.kw);
        }
        if (instance.ke) {
    ...
    F: function(xl, xr, kl, kr) {
      ...
      // Apply S-box transformations (standard Camellia pattern)
      // Each SP box corresponds to a specific S-box and byte position
      const y1 = (Camellia.SP1110[t1] ^ Camellia.SP0222[t2] ^ Camellia.SP3033[t3] ^ Camellia.SP4404[t4]) >>> 0;
      const y2 = (Camellia.SP4404[t5] ^ Camellia.SP1110[t6] ^ Camellia.SP0222[t7] ^ Camellia.SP3033[t8]) >>> 0;
      
      // P-function: linear transformation (standard Camellia)
      const yl = y1;
      let yr = y2;
      
      // Apply the standard Camellia linear transformation
      yr = ((yr << 8) | (yr >>> 24)) >>> 0; // ROL8 (rotate left by 8)
      const z1 = (yl ^ yr) >>> 0;
      yr = ((yr << 8) | (yr >>> 24)) >>> 0; // ROL8 again (total ROL16)
      yr = (yr ^ yl) >>> 0;
      
      return [z1, yr];
    },
    
    // FL and FLINV functions (based on LibGCrypt implementation)
    FLS: function(ll, lr, rl, rr, kll, klr, krl, krr) {
      const t0 = kll & ll;
      lr ^= Camellia.RL1(t0);
      const t1 = klr | lr;
      ll ^= t1;
    ...
    // FL inverse function
    FLINV: function(ll, lr, rl, rr, kll, klr, krl, krr) {
      const t2 = krl & rl;
      rr ^= Camellia.RL1(t2);
      const t3 = krr | rr;
      rl ^= t3;
    ...
    encryptBlock: function(id, plaintext) {
      ...
      // Main Feistel rounds (RFC 3713 structure)
      for (let i = 0; i < instance.rounds; i++) {
        // Feistel F-function round
        const roundKeyIndex = 4 + i * 2; // Start from subkey[4]
        const [F1, F2] = Camellia.F(D1, D2, subkey[roundKeyIndex], subkey[roundKeyIndex + 1]);
        const temp1 = (D3 ^ F1) >>> 0;
        const temp2 = (D4 ^ F2) >>> 0;
        ...
        // FL/FLINV layers after rounds 6 and 12 (for 18-round version)
        if (i === 5) { // After round 6 (ke1, ke2, ke3, ke4)
          [D1, D2, D3, D4] = Camellia.FLS(D1, D2, D3, D4, 
                                          subkey[12], subkey[13], 
                                          subkey[14], subkey[15]);
        } else if (i === 11) { // After round 12 (ke5, ke6, k17, k18)
          [D1, D2, D3, D4] = Camellia.FLS(D1, D2, D3, D4, 
                                          subkey[24], subkey[25], 
                                          subkey[24], subkey[25]); // Fix: should use ke5, ke6 for both pairs
        }
      }
      ...
      // Post-whitening (final XOR with kw5, kw6, kw7, kw8)
      D1 ^= subkey[28]; // kw5
      D2 ^= subkey[29]; // kw6
      D3 ^= subkey[30]; // kw7
      D4 ^= subkey[31]; // kw8
      
      return global.OpCodes.Words32BEToString([D1, D2, D3, D4]);
    },
    ...
    decryptBlock: function(id, ciphertext) {
      ...
      D1 ^= subkey[4 + instance.rounds * 2];     // kw5
      D2 ^= subkey[4 + instance.rounds * 2 + 1]; // kw6
      D3 ^= subkey[4 + instance.rounds * 2 + 2]; // kw7
      D4 ^= subkey[4 + instance.rounds * 2 + 3]; // kw8
      
      // Initial swap for decryption
      [D1, D2, D3, D4] = [D3, D4, D1, D2];
      
      // Main Feistel rounds (reverse)
      for (let i = instance.rounds - 1; i >= 0; i--) {
        // Feistel F-function (reverse)
        const [F1, F2] = Camellia.F(D3, D4, subkey[4 + i * 2], subkey[4 + i * 2 + 1]);
        const temp1 = D1 ^ F1;
        const temp2 = D2 ^ F2;
        ...
        // FL/FLINV every 6 rounds (reverse)
        if ((i + 1) % 6 === 0 && i < instance.rounds - 1) {
          const keIndex = 4 + Math.floor(i / 6) * 4;
          [D1, D2, D3, D4] = Camellia.FLINV(D1, D2, D3, D4, 
                                            subkey[keIndex], subkey[keIndex + 1], 
                                            subkey[keIndex + 2], subkey[keIndex + 3]);
        }
      }
    ...
    ROLDQ: function(ll, lr, rl, rr, bits) {
      if (bits >= 32) {
        return Camellia.ROLDQo32(ll, lr, rl, rr, bits);
      }
    ...
    ROLDQo32: function(ll, lr, rl, rr, bits) {
      const w0 = ll;
      const w1 = lr;
    ...
    // Instance class
    CamelliaInstance: function(key) {
      const keyBytes = global.OpCodes.StringToBytes(key);
      const keyLength = keyBytes.length;
      
      // Determine number of rounds based on key length
      this.rounds = (keyLength === 16) ? 18 : 24;
      
      // Generate subkeys using proper Camellia key schedule
      this.subkey = Camellia.generateKeySchedule(keyBytes, keyLength);
    },
  
    // Camellia key schedule generation (RFC 3713 compliant)
    generateKeySchedule: function(keyBytes, keyLength) {
      const subkey = new Array(68); // Maximum subkeys for 256-bit
      
      // Convert key to 32-bit words
      let kll = global.OpCodes.Pack32BE(keyBytes[0], keyBytes[1], keyBytes[2], keyBytes[3]);
      let klr = global.OpCodes.Pack32BE(keyBytes[4], keyBytes[5], keyBytes[6], keyBytes[7]);
      let krl = global.OpCodes.Pack32BE(keyBytes[8], keyBytes[9], keyBytes[10], keyBytes[11]);
      let krr = global.OpCodes.Pack32BE(keyBytes[12], keyBytes[13], keyBytes[14], keyBytes[15]);
      
      // For 128-bit keys, generate KA
      if (keyLength === 16) {
        return Camellia.generateKeySchedule128(kll, klr, krl, krr);
      } else {
    ...
    generateKeySchedule128: function(kll, klr, krl, krr) {
      ...
      const [f1l, f1r] = Camellia.F(kall, kalr, Camellia.SIGMA1L, Camellia.SIGMA1R);
      karl = (karl ^ f1l) >>> 0;
      karr = (karr ^ f1r) >>> 0;
      
      const [f2l, f2r] = Camellia.F(karl, karr, Camellia.SIGMA2L, Camellia.SIGMA2R);
      kall = (kall ^ f2l) >>> 0;
      kalr = (kalr ^ f2r) >>> 0;
      
      // Now generate round keys according to RFC 3713 Table 1
      ...
      // k5, k6: KL<<<15
      [kll, klr, krl, krr] = Camellia.ROLDQ(klOriginal[0], klOriginal[1], klOriginal[2], klOriginal[3], 15);
      subkey[8] = kll;
      subkey[9] = klr;
      ...
      [kall, kalr, karl, karr] = Camellia.ROLDQ(kall, kalr, karl, karr, 15);
      subkey[12] = kall;
      subkey[13] = kalr;
      ...
      // k9, k10: KA<<<15 (continued)
      subkey[16] = kall;
      subkey[17] = kalr;
      
      // k11, k12: KA<<<15 (continued)
      subkey[18] = karl;
      subkey[19] = karr;
      
      // k13, k14: KL<<<45
      [kll, klr, krl, krr] = Camellia.ROLDQ(klOriginal[0], klOriginal[1], klOriginal[2], klOriginal[3], 45);
      subkey[20] = kll;
      subkey[21] = klr;
      
      // k15, k16: KL<<<45 (continued)
      subkey[22] = krl;
      subkey[23] = krr;
      
      // ke5, ke6: KA<<<30 (FL/FLINV keys)
      [kall, kalr, karl, karr] = Camellia.ROLDQ(kall, kalr, karl, karr, 15); // KA<<<30 total
      subkey[24] = kall;
      subkey[25] = kalr;
      
      // k17, k18: KL<<<60
      [kll, klr, krl, krr] = Camellia.ROLDQ(klOriginal[0], klOriginal[1], klOriginal[2], klOriginal[3], 60);
      subkey[26] = kll;
      subkey[27] = klr;
      
      // kw5, kw6: KA<<<32 (final whitening)
      [kall, kalr, karl, karr] = Camellia.ROLDQ(kall, kalr, karl, karr, 2); // KA<<<32 total
      subkey[28] = kall;
      subkey[29] = kalr;
      
      // kw7, kw8: KA<<<32 (continued) (final whitening)
      subkey[30] = karl;
      subkey[31] = karr;
  ...
  if (global.Cipher && typeof global.Cipher.AddCipher === 'function') {
    global.Cipher.AddCipher(Camellia);
  }
  
  // Export to global scope
  global.Camellia = Camellia;
