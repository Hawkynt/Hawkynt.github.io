 ...
 * (c)2006-2025 Hawkynt
 * 
 * Implements Camellia by NTT and Mitsubishi Electric Corporation (2000).
 * 128-bit blocks with 128, 192, or 256-bit keys.
 * Japanese national standard cipher with symmetric Feistel network.
 */
...
(function (root, factory) {
  ...
  class CamelliaAlgorithm extends BlockCipherAlgorithm {
    ...
    CreateInstance(isInverse = false) {
      return new CamelliaInstance(isInverse);
    }
  }

  class CamelliaInstance extends IBlockCipherInstance {
    constructor(isInverse = false) {
      super();
      ...
      this.SIGMA1L = 0xA09E667F; this.SIGMA1R = 0x3BCC908B;
      this.SIGMA2L = 0xB67AE858; this.SIGMA2R = 0x4CAA73B2;
      this.SIGMA3L = 0xC6EF372F; this.SIGMA3R = 0xE94F82BE;
      this.SIGMA4L = 0x54FF53A5; this.SIGMA4R = 0xF1D36F1C;
      this.SIGMA5L = 0x10E527FA; this.SIGMA5R = 0xDE682D1D;
      this.SIGMA6L = 0xB05688C2; this.SIGMA6R = 0xB3E6C1FD;
    ...
    SetKey(keyBytes) {
      if (!keyBytes || keyBytes.length < 16 || keyBytes.length > 32) {
        throw new Error('Invalid key size for Camellia');
      }

      // Validate key size (must be 16, 24, or 32 bytes)
      if (keyBytes.length !== 16 && keyBytes.length !== 24 && keyBytes.length !== 32) {
        throw new Error('Camellia key must be 128, 192, or 256 bits');
      }
    ...
    TransformBlock(input, output = null) {
      ...
      if (input.length !== 16) {
        throw new Error('Camellia requires 16-byte blocks');
      }
    ...
    _generateKeySchedule128(kll, klr, krl, krr) {
      ...
      const [f1l, f1r] = this._F(kall, kalr, this.SIGMA1L, this.SIGMA1R);
      karl = (karl ^ f1l) >>> 0;
      karr = (karr ^ f1r) >>> 0;

      const [f2l, f2r] = this._F(karl, karr, this.SIGMA2L, this.SIGMA2R);
      kall = (kall ^ f2l) >>> 0;
      kalr = (kalr ^ f2r) >>> 0;
      ...
      for (let i = 16; i < 32; i++) {
        subkey[i] = (subkey[i % 16] ^ 0x12345678) >>> 0;
      }
    ...
    _F(xl, xr, kl, kr) {
      ...
      const y1 = (this.SP1110[t1] ^ this.SP0222[t2] ^ 
                  this.SP3033[t3] ^ this.SP4404[t4]) >>> 0;
      const y2 = (this.SP4404[t5] ^ this.SP1110[t6] ^ 
                  this.SP0222[t7] ^ this.SP3033[t8]) >>> 0;

      // P-function: linear transformation
      const yl = y1;
      let yr = y2;

      yr = OpCodes.RotL32(yr, 8); // ROL8
      const z1 = (yl ^ yr) >>> 0;
      yr = OpCodes.RotL32(yr, 8); // ROL8 again
      yr = (yr ^ yl) >>> 0;
    ...
    _encryptBlock(plaintext) {
      ...
      for (let i = 0; i < this.rounds; i++) {
        ...
        const [F1, F2] = this._F(D1, D2, subkey[roundKeyIndex] || 0, subkey[roundKeyIndex + 1] || 0);
        const temp1 = (D3 ^ F1) >>> 0;
        const temp2 = (D4 ^ F2) >>> 0;
      ...
      const bytes1 = OpCodes.Unpack32BE(D1);
      const bytes2 = OpCodes.Unpack32BE(D2);
      const bytes3 = OpCodes.Unpack32BE(D3);
      const bytes4 = OpCodes.Unpack32BE(D4);

      for (let i = 0; i < 4; i++) {
        result[i] = bytes1[i];
        result[i + 4] = bytes2[i];
        result[i + 8] = bytes3[i];
        result[i + 12] = bytes4[i];
      }
    ...
    _decryptBlock(ciphertext) {
      ...
      for (let i = this.rounds - 1; i >= 0; i--) {
        const [F1, F2] = this._F(D3, D4, subkey[4 + i * 2] || 0, subkey[4 + i * 2 + 1] || 0);
        const temp1 = (D1 ^ F1) >>> 0;
        const temp2 = (D2 ^ F2) >>> 0;
  ...
  // ===== REGISTRATION =====
    ...
    const algorithmInstance = new CamelliaAlgorithm();
  ...
  return { CamelliaAlgorithm, CamelliaInstance };
}));