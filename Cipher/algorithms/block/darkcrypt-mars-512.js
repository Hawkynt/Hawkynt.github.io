/*
 * MARS-512 (DarkCrypt) Implementation
 ...
 * backward mixing, plus whitening -- is standard textbook MARS.
 *
 * This variant extends the key to a 512-bit (64-byte) key, beyond the official MARS specification's 448-bit (56-byte) maximum.
 *
 * The official MARS specification only defines Nk in [4,14] 32-bit key
 * words (128-448 bit keys); this variant generalizes the same key-expansion
 * procedure (E-box stirring over an LFSR-like word array, seeded from the
 * key words via modulo indexing) to Nk=16 words (64-byte / 512-bit key)
 * with no upper bound enforced, and applies the standard
...
(function (root, factory) {
  ...
  class DarkCryptMARS512Algorithm extends BlockCipherAlgorithm {
    constructor() {
      ...
      this.name = "MARS-512 (DarkCrypt)";
      this.description = "MARS variant from the DarkCrypt Total Commander plugin: standard MARS round structure (8 forward mixing + 16 keyed core + 8 backward mixing rounds) with the key expansion generalized to a 64-byte (512-bit) key via modulo indexing into the key-word array, beyond the official spec's 56-byte maximum.";
      ...
      this.knownVulnerabilities = [
        new Vulnerability("Non-standard key size", "512-bit key exceeds the official MARS specification's 448-bit maximum; the key-expansion generalization used to support it (arbitrary Nk via modulo indexing) is an unanalyzed DarkCrypt-specific extension.", "Use AES or another vetted cipher.")
      ];

      // Test vectors verified against the DarkCrypt implementation.
      this.tests = [
        {
          text: "DarkCrypt Mars-512 — zero key/plaintext",
          ...
          expected: OpCodes.Hex8ToBytes("3f994f870ea5d1f13e593d5848ea71cb")
        },
        {
          text: "DarkCrypt Mars-512 — incrementing key/plaintext",
          uri: "https://totalcmd.net/plugring/darkcrypttc.html",
          ...
          expected: OpCodes.Hex8ToBytes("8d199c945f9b6dee2b61acb42e6607a1")
        },
        {
          text: "DarkCrypt Mars-512 — shifted incrementing key/plaintext",
          uri: "https://totalcmd.net/plugring/darkcrypttc.html",
  ...
  const algorithmInstance = new DarkCryptMARS512Algorithm();
  if (!AlgorithmFramework.Find(algorithmInstance.name)) {
    RegisterAlgorithm(algorithmInstance);
  }

  return { DarkCryptMARS512Algorithm, DarkCryptMARS512Instance };
}));
