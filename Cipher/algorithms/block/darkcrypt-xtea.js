/*
 * XTEA (DarkCrypt variant) Implementation
 * Compatible with AlgorithmFramework
 * (c)2006-2025 Hawkynt
 *
 * The XTEA block cipher as implemented in the DarkCrypt Total Commander plugin
 * (Alexander Myasnikov, "Zarya" project). It differs from textbook XTEA:
 *   - shift amounts are 6 and 9 (textbook XTEA uses 4 and 5)
 *   - 38 rounds (textbook XTEA uses 32)
...
(function (root, factory) {
  ...
  class DarkCryptXTEAAlgorithm extends BlockCipherAlgorithm {
    constructor() {
      super();

      this.name = "XTEA (DarkCrypt)";
      this.description = "XTEA variant from the DarkCrypt Total Commander plugin: shift amounts 6/9 (vs textbook 4/5), 38 rounds, little-endian words. 64-bit block, 128-bit key.";
      ...
      this.tests = [
        {
          text: "DarkCrypt Xtea — zero key/plaintext",
        ...
        {
          text: "DarkCrypt Xtea — incrementing key/plaintext",
          uri: "https://totalcmd.net/plugring/darkcrypttc.html",
        ...
        {
          text: "DarkCrypt Xtea — shifted incrementing key/plaintext",
          uri: "https://totalcmd.net/plugring/darkcrypttc.html",
    ...
    CreateInstance(isInverse = false) {
      return new DarkCryptXTEAInstance(this, isInverse);
    }
  }

  class DarkCryptXTEAInstance extends IBlockCipherInstance {
    ...
    set key(keyBytes) {
      ...
      if (keyBytes.length !== 16)
        throw new Error(`Invalid key size: ${keyBytes.length} bytes. XTEA (DarkCrypt) requires exactly 16 bytes`);
      this._key = [...keyBytes];
  ...
  const algorithmInstance = new DarkCryptXTEAAlgorithm();
  if (!AlgorithmFramework.Find(algorithmInstance.name)) {
    RegisterAlgorithm(algorithmInstance);
  }

  return { DarkCryptXTEAAlgorithm, DarkCryptXTEAInstance };
}));
