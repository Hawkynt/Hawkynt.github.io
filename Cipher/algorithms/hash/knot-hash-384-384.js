/*
 * KNOT-HASH-384-384 - NIST Lightweight Cryptography Finalist
 ...
 * based on bit-slice PRESENT-like permutations. KNOT-HASH-384-384 uses the
 * KNOT-384 permutation in sponge construction with 24-byte rate and 384-bit output.
 *
...
(function (root, factory) {
  ...
  const RATE = 6; // 6 bytes input rate for KNOT-HASH-384-384
  const ROUNDS = 104; // Number of permutation rounds (104 for hash-384-384)

  // Round constants for 7-bit variant (104 constants for 384-384)
  const RC7 = [
  ...
  /**
 * KnotHash384_384 - Cryptographic hash function
 * @class
 ...
 */
  ...
  class KnotHash384_384 extends HashFunctionAlgorithm {
    constructor() {
      ...
      this.description = "Lightweight hash function based on bit-sliced PRESENT-like permutations, finalist in NIST Lightweight Cryptography competition. Uses KNOT-384 permutation with 384-bit output.";
  ...
  /**
 * KnotHash384_384 cipher instance implementing Feed/Result pattern
 * @class
 ...
 */
  ...
  class KnotHash384_384Instance extends IHashFunctionInstance {
    ...
    Reset() {
      // Initialize state to all zeros (no domain separator for 384-384)
   ...
   */
    ...
    Result() {
      ...
      // Squeeze first third of output (24 bytes)
      var output = [];
      for (var i = 0; i < 24; i++) {
      ...
      // Squeeze second third of output (24 bytes)
      for (var i = 0; i < 24; i++) {
        output.push(this.state[i]);
      }
  ...
  // Register algorithm
  RegisterAlgorithm(new KnotHash384_384());
