/*
 * SHA-512/256 Hash Function - Universal AlgorithmFramework Implementation
 * FIPS 180-4 Section 5.3.6 - SHA-512 with modified IV and 256-bit output
 * (c)2006-2025 Hawkynt
 */
...
(function (root, factory) {
  ...
  // SHA-512/256 initial hash values (from FIPS 180-4 Section 5.3.6.2)
  // These are derived using: SHA-512(SHA-512/256) with modified initial values
  ...
  class SHA512_256Algorithm extends HashFunctionAlgorithm {
    constructor() {
      super();
    ...
    CreateInstance(isInverse = false) {
      return new SHA512_256AlgorithmInstance(this, isInverse);
    }
  }

  class SHA512_256AlgorithmInstance extends IHashFunctionInstance {
    constructor(algorithm, isInverse = false) {
      super(algorithm);
      this.isInverse = isInverse;
      this.OutputSize = 32; // 256 bits = 32 bytes

      // SHA-512/256 state variables
      this._h = null;
      this._buffer = null;
      this._length = 0;
    ...
    /**
     * Initialize the hash state with SHA-512/256 initial values
     ...
     * @returns {Array} Hash digest as byte array (256 bits = 32 bytes)
     */
    Final() {
      ...
      // Convert hash to bytes (big-endian) - TRUNCATED to 256 bits (32 bytes)
      const result = [];

      // Output first 4 complete 64-bit words (32 bytes)
      for (let i = 0; i < 4; i++) {
    ...
    DecryptBlock(blockIndex, ciphertext) {
      ...
      throw new Error('SHA-512/256 is a one-way hash function - decryption not possible');
    }
  ...
  const algorithmInstance = new SHA512_256Algorithm();
  ...
  return { SHA512_256Algorithm, SHA512_256AlgorithmInstance };
}));
