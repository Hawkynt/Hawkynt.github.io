/*
 * LSH-224 Hash Function - Korean Lightweight Secure Hash
 * Professional implementation matching Crypto++ reference
 * (c)2006-2025 Hawkynt
 *
 * LSH-224 produces 224-bit digests using Korean cryptographic standard
...
(function (root, factory) {
  ...
  // LSH224 Initial Values (different from LSH256)
  ...
  // LSH256 Step Constants (shared with LSH-256)
  ...
  class LSH224Algorithm extends HashFunctionAlgorithm {
    ...
    CreateInstance(isInverse = false) {
      ...
      return new LSH224Instance(this);
    }
  }

  class LSH224Instance extends IHashFunctionInstance {
    ...
    _initialize() {
      // Load IV224 (different from LSH-256)
      for (let i = 0; i < 8; i++) {
        this.cv_l[i] = LSH256_IV224[i];
        this.cv_r[i] = LSH256_IV224[i + 8];
      }
    ...
    Result() {
      ...
      // Extract 224-bit hash (28 bytes) from cv_l
  ...
  const algorithmInstance = new LSH224Algorithm();
  if (!AlgorithmFramework.Find(algorithmInstance.name)) {
    RegisterAlgorithm(algorithmInstance);
  }

  return { LSH224Algorithm, LSH224Instance };
}));
