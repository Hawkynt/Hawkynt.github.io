 ...
 * Designed by Özgül Küçük (KU Leuven, Belgium / Turkey)
 ...
 *
 * Algorithm: Concatenate-Permute-Truncate construction with an
 * AES/Serpent-inspired non-linear permutation and a linear-code based
 * message expansion (bit-level table T256 for the small variants,
 * T512 for the big variants).
 * Variants: Hamsi-224, Hamsi-256 (small, 4-byte blocks, 8x32-bit state)
...
(function (root, factory) {
  ...
  // Message expansion code T256 for Hamsi-224/256 (small variants).
  // 32 rows: one per (byte index 0..3) x (bit index 0..7, LSB first).
  ...
  // Message expansion code T512 for Hamsi-384/512 (big variants).
  ...
  // Message expansion: XOR the table row for every set bit of the
  ...
  function sbox(s, ia, ib, ic, id) {
    ...
    d = OpCodes.Xor32(OpCodes.Or32(d, t), b);
    t = OpCodes.Xor32(t, c);
    b = d;
    d = OpCodes.Xor32(OpCodes.Or32(d, t), a);
    a = OpCodes.And32(a, b);
    t = OpCodes.Xor32(t, a);
    b = OpCodes.Xor32(b, d);
    b = OpCodes.Xor32(b, t);
  ...
  function linearDiffusion(s, ia, ib, ic, id) {
    ...
    s[ib] = OpCodes.Xor32(OpCodes.Xor32(s[ib], s[ia]), s[ic]);
    s[id] = OpCodes.Xor32(OpCodes.Xor32(s[id], s[ic]), OpCodes.Shl32(s[ia], 3));
    s[ib] = OpCodes.RotL32(s[ib], 1);
    s[id] = OpCodes.RotL32(s[id], 7);
    s[ia] = OpCodes.Xor32(OpCodes.Xor32(s[ia], s[ib]), s[id]);
    s[ic] = OpCodes.Xor32(OpCodes.Xor32(s[ic], s[id]), OpCodes.Shl32(s[ib], 7));
    s[ia] = OpCodes.RotL32(s[ia], 5);
    s[ic] = OpCodes.RotL32(s[ic], 22);
  }
  ...
  function compressSmall(h, buf, offset, rounds, alpha) {
    ...
    for (let r = 0; r < rounds; ++r) {
      ...
      s[0] = OpCodes.Xor32(s[0], alpha[0x00]);
      s[1] = OpCodes.Xor32(OpCodes.Xor32(s[1], alpha[0x01]), r);
      s[2] = OpCodes.Xor32(s[2], alpha[0x02]);
      s[3] = OpCodes.Xor32(s[3], alpha[0x03]);
      s[4] = OpCodes.Xor32(s[4], alpha[0x08]);
      s[5] = OpCodes.Xor32(s[5], alpha[0x09]);
      s[6] = OpCodes.Xor32(s[6], alpha[0x0A]);
      s[7] = OpCodes.Xor32(s[7], alpha[0x0B]);
      s[8] = OpCodes.Xor32(s[8], alpha[0x10]);
      s[9] = OpCodes.Xor32(s[9], alpha[0x11]);
      s[10] = OpCodes.Xor32(s[10], alpha[0x12]);
      s[11] = OpCodes.Xor32(s[11], alpha[0x13]);
      s[12] = OpCodes.Xor32(s[12], alpha[0x18]);
      s[13] = OpCodes.Xor32(s[13], alpha[0x19]);
      s[14] = OpCodes.Xor32(s[14], alpha[0x1A]);
      s[15] = OpCodes.Xor32(s[15], alpha[0x1B]);
    ...
    // Finalize by XORing state into hash (concatenation-truncation mode)
    h[0] = OpCodes.Xor32(h[0], s[0]);
    h[1] = OpCodes.Xor32(h[1], s[1]);
    h[2] = OpCodes.Xor32(h[2], s[2]);
    h[3] = OpCodes.Xor32(h[3], s[3]);
    h[4] = OpCodes.Xor32(h[4], s[8]);
    h[5] = OpCodes.Xor32(h[5], s[9]);
    h[6] = OpCodes.Xor32(h[6], s[10]);
    h[7] = OpCodes.Xor32(h[7], s[11]);
  }
  ...
  function compressBig(h, buf, offset, rounds, alpha) {
    ...
    s[8] = h[4];   s[9] = h[5];   s[10] = m[4];  s[11] = m[5];
    s[12] = h[6];  s[13] = h[7];  s[14] = m[6];  s[15] = m[7];
    s[16] = m[8];  s[17] = m[9];  s[18] = h[8];  s[19] = h[9];
    s[20] = m[10]; s[21] = m[11]; s[22] = h[10]; s[23] = h[11];
    s[24] = h[12]; s[25] = h[13]; s[26] = m[12]; s[27] = m[13];
    s[28] = h[14]; s[29] = h[15]; s[30] = m[14]; s[31] = m[15];

    for (let r = 0; r < rounds; ++r) {
      // Add round constants
      for (let i = 0; i < 32; ++i) {
        s[i] = OpCodes.Xor32(s[i], alpha[i]);
      }
      s[1] = OpCodes.Xor32(s[1], r);
      ...
      // Linear diffusion layer (12 applications, per the ROUND_BIG macro)
      linearDiffusion(s, 0, 9, 18, 27);
      linearDiffusion(s, 1, 10, 19, 28);
      linearDiffusion(s, 2, 11, 20, 29);
      linearDiffusion(s, 3, 12, 21, 30);
      linearDiffusion(s, 4, 13, 22, 31);
      linearDiffusion(s, 5, 14, 23, 24);
      linearDiffusion(s, 6, 15, 16, 25);
      linearDiffusion(s, 7, 8, 17, 26);
      linearDiffusion(s, 0, 2, 5, 7);
      linearDiffusion(s, 16, 19, 21, 22);
      linearDiffusion(s, 9, 11, 12, 14);
      linearDiffusion(s, 25, 26, 28, 31);
    }
    ...
    h[2] = OpCodes.Xor32(h[2], s[2]);
    h[3] = OpCodes.Xor32(h[3], s[3]);
    h[4] = OpCodes.Xor32(h[4], s[4]);
    h[5] = OpCodes.Xor32(h[5], s[5]);
    h[6] = OpCodes.Xor32(h[6], s[6]);
    h[7] = OpCodes.Xor32(h[7], s[7]);
    h[8] = OpCodes.Xor32(h[8], s[16]);
    h[9] = OpCodes.Xor32(h[9], s[17]);
    h[10] = OpCodes.Xor32(h[10], s[18]);
    h[11] = OpCodes.Xor32(h[11], s[19]);
    h[12] = OpCodes.Xor32(h[12], s[20]);
    h[13] = OpCodes.Xor32(h[13], s[21]);
    h[14] = OpCodes.Xor32(h[14], s[22]);
    h[15] = OpCodes.Xor32(h[15], s[23]);
  }
  ...
  class Hamsi224 extends HashFunctionAlgorithm {
    constructor() {
      ...
      this.documentation = [
        ...
        new LinkItem("Hamsi Submission Package (NIST SHA-3 Round 2)", "https://csrc.nist.gov/CSRC/media/Projects/Hash-Functions/documents/Hamsi_Round2.zip"),
      ...
      this.references = [
        new LinkItem("Distinguishers for the Compression Function and Output Transformation of Hamsi-256", "https://eprint.iacr.org/2010/091.pdf"),
 ...
 */
  ...
  class Hamsi256 extends HashFunctionAlgorithm {
    constructor() {
      super();
      this.name = "Hamsi-256";
      this.description = "SHA-3 candidate hash function with 256-bit output. Uses a Serpent-inspired non-linear permutation with a linear-code based message expansion in a concatenation-truncation construction. Known distinguishers exist against the compression function and output transformation.";
      this.inventor = "Özgül Küçük";
      this.year = 2008;
      this.category = CategoryType.HASH;
      this.subCategory = "Hash Function";
 ...
 */
  ...
  class Hamsi384 extends HashFunctionAlgorithm {
    constructor() {
      super();
      this.name = "Hamsi-384";
      this.description = "SHA-3 candidate hash function with 384-bit output. Uses a Serpent-inspired non-linear permutation with a linear-code based message expansion in a concatenation-truncation construction.";
      this.inventor = "Özgül Küçük";
      this.year = 2008;
      this.category = CategoryType.HASH;
      this.subCategory = "Hash Function";
 ...
 */
  ...
  class Hamsi512 extends HashFunctionAlgorithm {
    constructor() {
      super();
      this.name = "Hamsi-512";
      this.description = "SHA-3 candidate hash function with 512-bit output. Uses a Serpent-inspired non-linear permutation with a linear-code based message expansion in a concatenation-truncation construction.";
      this.inventor = "Özgül Küçük";
      this.year = 2008;
      this.category = CategoryType.HASH;
      this.subCategory = "Hash Function";
