...
(function(global) {
  ...
  class SubterraneanHashInstance extends IHashFunctionInstance {
     ...
     */
    _round() {
      let x0 = this.state[0] >>> 0;
      let x1 = this.state[1] >>> 0;
      let x2 = this.state[2] >>> 0;
      let x3 = this.state[3] >>> 0;
      let x4 = this.state[4] >>> 0;
      let x5 = this.state[5] >>> 0;
      let x6 = this.state[6] >>> 0;
      let x7 = this.state[7] >>> 0;
      let x8 = this.state[8] >>> 0;
      ...
      this.state[0] = (BCP(x0,  0)     ^ BDN(x0, 12,  1) ^ BDN(x0, 24,  2) ^
                       ...
                       BUP(x3,  7, 30) ^ BUP(x3, 19, 31)) >>> 0;

      this.state[1] = (BDN(x3, 31,  0) ^ BDN(x4, 11,  1) ^ BDN(x4, 23,  2) ^
                       ...
                       BUP(x7,  6, 30) ^ BUP(x7, 18, 31)) >>> 0;

      this.state[2] = (BDN(x7, 30,  0) ^ BDN(x0,  9,  1) ^ BDN(x0, 21,  2) ^
                       ...
                       BUP(x3,  4, 30) ^ BUP(x3, 16, 31)) >>> 0;

      this.state[3] = (BDN(x3, 28,  0) ^ BDN(x4,  8,  1) ^ BDN(x4, 20,  2) ^
                       ...
                       BUP(x7,  3, 30) ^ BUP(x7, 15, 31)) >>> 0;

      this.state[4] = (BDN(x7, 27,  0) ^ BDN(x0,  6,  1) ^ BDN(x0, 18,  2) ^
                       ...
                       BUP(x3,  1, 30) ^ BUP(x3, 13, 31)) >>> 0;

      this.state[5] = (BDN(x3, 25,  0) ^ BDN(x4,  5,  1) ^ BDN(x4, 17,  2) ^
                       ...
                       BUP(x7,  0, 30) ^ BUP(x7, 12, 31)) >>> 0;

      this.state[6] = (BDN(x7, 24,  0) ^ BDN(x0,  3,  1) ^ BDN(x0, 15,  2) ^
                       ...
                       BCP(x2, 30)     ^ BUP(x3, 10, 31)) >>> 0;

      this.state[7] = (BDN(x3, 22,  0) ^ BDN(x4,  2,  1) ^ BDN(x4, 14,  2) ^
                       ...
                       BUP(x6, 29, 30) ^ BUP(x7,  9, 31)) >>> 0;

      this.state[8] = BDN(x7, 21, 0) >>> 0;
    }
    ...
    _absorbByte(dataByte) {
      const x = dataByte & 0xFF;

      // Rearrange bits and absorb into state (specific to Subterranean's structure)
      this.state[0] = (this.state[0] ^ ((x << 1) & 0x00000002)) >>> 0;
      this.state[1] = (this.state[1] ^ (x & 0x00000008)) >>> 0;
      this.state[2] = (this.state[2] ^ 0x00000001) >>> 0; // 9th padding bit is always 1
      this.state[4] = (this.state[4] ^ (((x << 6) & 0x00000100) ^ ((x << 1) & 0x00000040))) >>> 0;
      this.state[5] = (this.state[5] ^ ((x << 15) & 0x00010000)) >>> 0;
      this.state[6] = (this.state[6] ^ ((x >>> 1) & 0x00000020)) >>> 0;
      this.state[7] = (this.state[7] ^ (((x << 21) & 0x02000000) ^ ((x << 3) & 0x00000400))) >>> 0;
    }
    ...
    _duplexZero() {
      this._round();
      this.state[0] = (this.state[0] ^ 0x02) >>> 0; // Padding for empty block
    }
    ...
    _blank() {
      for (let round = 0; round < 8; ++round) {
        this._round();
        this.state[0] = (this.state[0] ^ 0x02) >>> 0; // Padding bit
      }
    }
    ...
    _extract() {
      ...
      const x0 = this.state[0] >>> 0;
      const x1 = this.state[1] >>> 0;
      const x2 = this.state[2] >>> 0;
      const x3 = this.state[3] >>> 0;
      const x4 = this.state[4] >>> 0;
      const x5 = this.state[5] >>> 0;
      const x6 = this.state[6] >>> 0;
      const x7 = this.state[7] >>> 0;
