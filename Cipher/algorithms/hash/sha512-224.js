/*
 * SHA-512/224 Hash Function - Universal AlgorithmFramework Implementation
 * FIPS 180-4 Section 5.3.6 - SHA-512 with modified IV and 224-bit output
 * (c)2006-2025 Hawkynt
 */
...
(function (root, factory) {
  ...
  // SHA-512/224 initial hash values (from FIPS 180-4 Section 5.3.6.1)
  // These are derived using: SHA-512(SHA-512/224) with modified initial values
  ...
  // SHA-512 round constants (same as SHA-512)
  ...
  class SHA512_224Algorithm extends HashFunctionAlgorithm {
    constructor() {
      super();
      ...
      this.blockSize = 128; // 1024 bits = 128 bytes (same as SHA-512)
      ...
      // Test vectors from FIPS 180-4 via BouncyCastle
      this.tests = [
        {
    ...
    CreateInstance(isInverse = false) {
      return new SHA512_224AlgorithmInstance(this, isInverse);
    }
  }

  class SHA512_224AlgorithmInstance extends IHashFunctionInstance {
    constructor(algorithm, isInverse = false) {
      super(algorithm);
      this.isInverse = isInverse;
      this.OutputSize = 28; // 224 bits = 28 bytes

      // SHA-512/224 state variables
      this._h = null;
      this._buffer = null;
      this._length = 0;
    ...
    /**
     * Initialize the hash state with SHA-512/224 initial values
     ...
     * @returns {Array} Hash digest as byte array (224 bits = 28 bytes)
     */
    Final() {
      ...
      // If we've exceeded 112 bytes, need another block
      ...
      // Convert hash to bytes (big-endian) - TRUNCATED to 224 bits (28 bytes)
      const result = [];

      // Output first 3 complete 64-bit words (24 bytes)
      for (let i = 0; i < 3; i++) {
      ...
      // Output first 4 bytes of the 4th word (for total of 28 bytes)
      const value = this._h[3];
      for (let j = 0; j < 4; j++) {
        result.push(Number((value >> BigInt((7 - j) * 8)) & 0xFFn));
      }
     ...
     * NIST FIPS 180-4 Section 6.4.2 (same as SHA-512)
    ...
    DecryptBlock(blockIndex, ciphertext) {
      ...
      throw new Error('SHA-512/224 is a one-way hash function - decryption not possible');
    }
  ...
  const algorithmInstance = new SHA512_224Algorithm();
  ...
  return { SHA512_224Algorithm, SHA512_224AlgorithmInstance };
}));
